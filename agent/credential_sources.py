"""Unified removal contract for every credential source Gabru reads from.

Gabru seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.gabru/.env
    claude_code   — ~/.claude/.credentials.json
    gabru_pkce   — ~/.gabru/.anthropic_oauth.json
    ...
    manual        — user ran `gabru auth add`
...
unify here is **removal**:
    ...
    ``gabru auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), gabru_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.gabru/.env  → clear it
    ...
    """
    from gabru_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Gabru.",
            f"  The pool entry is now suppressed — Gabru will ignore "
            f"{env_var} until you run `gabru auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Gabru stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `gabru auth add anthropic` to re-enable if needed.",
    ])


def _remove_gabru_pkce(provider: str, removed) -> RemovalResult:
    """~/.gabru/.anthropic_oauth.json is ours — delete it outright."""
    from gabru_constants import get_gabru_home

    result = RemovalResult()
    oauth_file = get_gabru_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Gabru Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `gabru login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `gabru auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Gabru auth store is not enough — _seed_from_singletons() would
    ...
    via ``gabru auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from gabru_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `gabru auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `gabru auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Gabru stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `gabru auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="gabru_pkce",
        remove_fn=_remove_gabru_pkce,
        description="~/.gabru/.anthropic_oauth.json",
    ))
    register(RemovalStep(
