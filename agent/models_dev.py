...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.gabru/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Gabru ↔ models.dev
# ---------------------------------------------------------------------------

# Gabru provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Gabru (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_gabru_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Gabru provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Gabru agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Gabru provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Gabru ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Gabru or models.dev provider ID.  Tries exact match then
