...
# Matches ${GABRU_SKILL_DIR} / ${GABRU_SESSION_ID} tokens in SKILL.md.
# Tokens that don't resolve (e.g. ${GABRU_SESSION_ID} with no session) are
# left as-is so the user can debug them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(GABRU_SKILL_DIR|GABRU_SESSION_ID)\}")
...
def _substitute_template_vars(
    ...
    """Replace ${GABRU_SKILL_DIR} / ${GABRU_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "GABRU_SKILL_DIR" and skill_dir_str:
            return skill_dir_str
        if token == "GABRU_SESSION_ID" and session_id:
...
def build_plan_path(
    ...
    *,
    ...
    workspace instead of the Gabru host's global home directory.
    """
    ...
    return Path(".gabru") / "plans" / f"{timestamp}-{slug}.md"
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.gabru.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_gabru_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.gabru/skills/ and return a mapping of /command -> skill info.
