...
from gabru_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``GABRU_PLATFORM`` or
            ``GABRU_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.gabru/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    gabru = metadata.get("gabru") or {}
    if not isinstance(gabru, dict):
        gabru = {}
    return {
        "fallback_for_toolsets": gabru.get("fallback_for_toolsets", []),
        "requires_toolsets": gabru.get("requires_toolsets", []),
        "fallback_for_tools": gabru.get("fallback_for_tools", []),
        "requires_tools": gabru.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    gabru = metadata.get("gabru")
    if not isinstance(gabru, dict):
        return []
    raw = gabru.get("config")
    if not raw:
        return []
