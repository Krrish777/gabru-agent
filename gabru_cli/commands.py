"""Slash command definitions and autocomplete for the Gabru CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Gabru config/state", "Session",
    ...
    CommandDef("update", "Update Gabru Agent to the latest version", "Info",
               gateway_only=True),
...
def _collect_gateway_skill_entries(
    ...
    try:
        from gabru_cli.plugins import get_plugin_commands
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``gabru skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /gabru handler.

    Maps both canonical names and aliases so /gabru bg do stuff works
    the same as /gabru background do stuff.
    """
    overrides = _resolve_config_gates()
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from gabru_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
