"""Shell completion script generation for gabru CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_gabru_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Gabru Agent bash completion
# Add to ~/.bashrc:
#   eval "$(gabru completion bash)"

_gabru_profiles() {{
    local profiles_dir="$HOME/.gabru/profiles"
...
_gabru_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_gabru_profiles)" -- "$cur"))
        return
    fi
...
complete -F _gabru_completion gabru
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef gabru
# Gabru Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(gabru completion zsh)"

_gabru_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.gabru/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.gabru/profiles 2>/dev/null)}}")
    fi
...
_gabru() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_gabru_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'gabru command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Gabru Agent fish completion",
        "# Add to your config:",
        "#   gabru completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __gabru_profiles",
        "    echo default",
        "    if test -d $HOME/.gabru/profiles",
        "        ls $HOME/.gabru/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c gabru -f -s p -l profile"
        " -d 'Profile name' -xa '(__gabru_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__gabru_profiles)' -d 'Profile name'"
                )
