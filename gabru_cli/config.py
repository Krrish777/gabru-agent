"""
Configuration management for Gabru Agent.

Config files are stored in ~/.gabru/ for easy access:
- ~/.gabru/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.gabru/.env         - API keys and secrets

This module provides:
- gabru config          - Show current configuration
- gabru config edit     - Open config in editor
- gabru config set      - Set a specific value
- gabru config wizard   - Re-run setup wizard
"""
...
from gabru_cli.colors import Colors, color
from gabru_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("GABRU_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_gabru_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Gabru is running in package-manager-managed mode.

    Two signals: the GABRU_MANAGED env var (set by the systemd service),
    or a .managed marker file in GABRU_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade gabru-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "gabru update"


def format_managed_message(action: str = "modify this Gabru installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("GABRU_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Gabru installation is managed by NixOS "
            f"(GABRU_MANAGED={env_hint}).\n"
            "Edit services.gabru-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Gabru installation is managed by Homebrew "
            f"(GABRU_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade gabru-agent"
        )

    return (
        f"Cannot {action}: this Gabru installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Gabru."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from GABRU_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, gabru_bin
    or None if container mode is not active, we're already inside the
    container, or GABRU_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("GABRU_DEV") == "1":
        return None

    from gabru_constants import is_container
    if is_container():
        return None

    container_mode_file = get_gabru_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "gabru-agent")
    exec_user = info.get("exec_user", "gabru")
    gabru_bin = info.get("gabru_bin", "/data/current-package/bin/gabru")
...
# Re-export from gabru_constants — canonical definition lives there.
from gabru_constants import get_gabru_home  # noqa: F811,E402


def get_config_path() -> Path:
    """Get the main config file path."""
    return get_gabru_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_gabru_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the gabru group can
    share state with the gateway service.

    The mode can be overridden via the GABRU_HOME_MODE environment variable
    (e.g. GABRU_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse GABRU_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("GABRU_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Gabru runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("GABRU_CONTAINER") or os.environ.get("GABRU_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set GABRU_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into GABRU_HOME if the user doesn't have one yet."""
...
def ensure_gabru_home():
    """Ensure ~/.gabru directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_gabru_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_gabru_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"GABRU_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # skipped. When empty, Gabru auto-appends ``~/.bashrc`` if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Gabru sources ``~/.bashrc`` in the login
        ...
        # docker_env lets you specify exact key-value pairs — useful when Gabru
        ...
        #  "/home/user/.gabru/cache/documents:/output"]
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Gabru sends a stable profile-scoped userId to Camofox
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.gabru/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.gabru/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${GABRU_SKILL_DIR} and ${GABRU_SESSION_ID} in SKILL.md
    ...
    # This section is only needed for gabru-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Gabru Agent*" header.
    ...
    # stored approval from ~/.gabru/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or GABRU_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via GABRU_CRON_MAX_PARALLEL env var.
    ...
    "code_execution": {
        ...
        #     gabru-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.gabru/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g.  -> firecrawl-gateway.",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Gabru auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @gabru:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. GABRU_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'gabru-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Gabru API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Gabru API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Gabru API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # GABRU_TOOL_PROGRESS and GABRU_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.gabru.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'gabru setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Gabru won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'gabru doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("GABRU_HOME", "~/.gabru")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("GABRU_TOOL_PROGRESS")
            old_mode = get_env_value("GABRU_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from GABRU_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from GABRU_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("GABRU_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from GABRU_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$GABRU_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_gabru_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `gabru plugins enable <name>` to activate."
                    )
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: gabru config set <key> <value>")
    ...
    # metadata.gabru.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.gabru/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.gabru/config.yaml."""
    ensure_gabru_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — gabru auth) — OpenAI Codex
#   nous         (OAuth — gabru auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.gabru/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.gabru/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Gabru env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.gabru/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.gabru/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.gabru/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.gabru/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Gabru — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Gabru vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.gabru/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Gabru Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  gabru config edit     # Edit config file", Colors.DIM))
    print(color("  gabru config set <key> <value>", Colors.DIM))
    print(color("  gabru setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: gabru config set <key> <value>")
            print()
            print("Examples:")
            print("  gabru config set model anthropic/claude-sonnet-4")
            print("  gabru config set terminal.backend docker")
            print("  gabru config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'gabru config migrate' to add them")
        ...
        print("  gabru config           Show current configuration")
        print("  gabru config edit      Open config in editor")
        print("  gabru config set <key> <value>   Set a config value")
        print("  gabru config check     Check for missing/outdated config")
        print("  gabru config migrate   Update config with new options")
        print("  gabru config path      Show config file path")
        print("  gabru config env-path  Show .env file path")
        sys.exit(1)
