#!/usr/bin/env python3
"""
Gabru CLI - Main entry point.

Usage:
    gabru                     # Interactive chat (default)
    gabru chat                # Interactive chat
    gabru gateway             # Run gateway in foreground
    gabru gateway start       # Start gateway as service
    gabru gateway stop        # Stop gateway service
    gabru gateway status      # Show gateway status
    gabru gateway install     # Install gateway service
    gabru gateway uninstall   # Uninstall gateway service
    gabru setup               # Interactive setup wizard
    gabru logout              # Clear stored authentication
    gabru status              # Show status of all components
    gabru cron                # Manage cron jobs
    gabru cron list           # List cron jobs
    gabru cron status         # Check if cron scheduler is running
    gabru doctor              # Check configuration and dependencies
    gabru honcho setup                    # Configure Honcho AI memory integration
    gabru honcho status                   # Show Honcho config and connection status
    gabru honcho sessions                 # List directory → session name mappings
    gabru honcho map <name>               # Map current directory to a session name
    gabru honcho peer                     # Show peer names and dialectic settings
    gabru honcho peer --user NAME         # Set user peer name
    gabru honcho peer --ai NAME           # Set AI peer name
    gabru honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    gabru honcho mode                     # Show current memory mode
    gabru honcho mode [hybrid|honcho|local]  # Set memory mode
    gabru honcho tokens                   # Show token budget settings
    gabru honcho tokens --context N       # Set session.context() token cap
    gabru honcho tokens --dialectic N     # Set dialectic result char cap
    gabru honcho identity                 # Show AI peer identity representation
    gabru honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    gabru honcho migrate                  # Step-by-step migration guide: OpenClaw native → Gabru + Honcho
    gabru version             Show version
    gabru update              Update to latest version
    gabru uninstall           Uninstall Gabru Agent
    gabru acp                 Run as an ACP server for editor integration
    gabru sessions browse     Interactive session picker with search

    gabru claw migrate --dry-run  # Preview migration without changes
"""
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to GABRU_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (gabru tools, gabru setup, gabru model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'gabru {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any gabru module import.
#
# Many modules cache GABRU_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("GABRU_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.gabru/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set GABRU_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the gabru root
    if profile_name is None:
        try:
            from gabru_constants import get_default_gabru_root

            active_path = get_default_gabru_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set GABRU_HOME
    if profile_name is not None:
        try:
            from gabru_cli.profiles import resolve_profile_env

            gabru_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent gabru from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["GABRU_HOME"] = gabru_home
...
# Load .env from ~/.gabru/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from gabru_cli.config import get_gabru_home
from gabru_cli.env_loader import load_gabru_dotenv

load_gabru_dotenv(project_env=PROJECT_ROOT / ".env")

# Initialize centralized file logging early — all `gabru` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from gabru_logging import setup_logging as _setup_logging
...
try:
    from gabru_cli.config import load_config as _load_config_early
    from gabru_constants import apply_ipv4_preference as _apply_ipv4
...
from gabru_cli import __version__, __release_date__
from gabru_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from gabru_cli.config import get_env_path, get_gabru_home, load_config
    from gabru_cli.auth import get_auth_status

    # Determine whether Gabru itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from gabru_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_gabru_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_gabru_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Gabru has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Gabru to use their tokens.
    if _has_gabru_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, gabru_bin
        cli_args: the original CLI arguments (everything after 'gabru')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    gabru_bin = container_info["gabru_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo gabru {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo gabru {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str]) -> None:
    ...
    print(f"  gabru --tui --resume {target}")
    if title:
        print(f'  gabru --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @gabru/ink is missing or node_modules is behind package-lock.json (post-pull)."""
    ink = root / "node_modules" / "@gabru" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    """Directory whose dist/entry.js we should run: GABRU_TUI_DIR first, else repo ui-tui."""
    env = os.environ.get("GABRU_TUI_DIR")
    if env:
        p = Path(env)
...
def _gabru_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "gabru-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``GABRU_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("GABRU_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    gabru_home = os.environ.get("GABRU_HOME") or str(Path.home() / ".gabru")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "GABRU_HOME": gabru_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(gabru_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (GABRU_TUI_DIR or ui-tui, build when stale)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("GABRU_NODE")
    ...
    # pre-built dist + node_modules (nix / full GABRU_TUI_DIR) skips npm.
    if not tui_dev:
        ext_dir = os.environ.get("GABRU_TUI_DIR")
        if ext_dir:
            p = Path(ext_dir)
            if (p / "dist" / "entry.js").exists() and not _tui_need_npm_install(p):
                node = _node_bin("node")
    ...
    if _tui_need_npm_install(tui_dir):
        if not os.environ.get("GABRU_QUIET"):
    ...
    if tui_dev:
        if _gabru_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/gabru-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
...
def _launch_tui(resume_session_id: Optional[str] = None, tui_dev: bool = False):
    ...
    env = os.environ.copy()
    env["GABRU_PYTHON_SRC_ROOT"] = os.environ.get(
        "GABRU_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("GABRU_PYTHON", sys.executable)
    env.setdefault("GABRU_CWD", os.getcwd())
    ...
    if resume_session_id:
        env["GABRU_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("GABRU_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'gabru sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Gabru isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  gabru setup")
        print()

        from gabru_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'gabru setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from gabru_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["GABRU_YOLO_MODE"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["GABRU_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from gabru_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from gabru_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Gabru?")
        print()
    ...
    session_dir = get_gabru_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: gabru gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  gabru gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Gabru Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  gabru gateway")
        ...
        print()
        print("  Or install as a service: gabru gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'gabru whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from gabru_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``gabru model``) and the setup wizard
    ...
    effective_provider = (
        config_provider or os.getenv("GABRU_INFERENCE_PROVIDER") or "auto"
    )
    try:
    ...
    from gabru_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.gabru/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.gabru/.env if the active provider is not 'custom'.
    ...
    """
    from gabru_cli.config import get_env_value, save_env_value, load_config
...
#
# Gabru uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `gabru model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `gabru model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    """
    from gabru_cli.config import load_config
    ...
    while True:
        ...
        print("  Gabru only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``gabru model`` flow, then route aux tasks to them here.
    """
    from gabru_cli.config import load_config
    from gabru_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from gabru_cli.auth import _prompt_model_selection
    from gabru_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from gabru_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from gabru_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from gabru_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from gabru_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from gabru_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from gabru_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `gabru auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from gabru_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from gabru_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    from gabru_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from gabru_cli.auth import _save_model_choice, deactivate_provider
    from gabru_cli.config import get_env_value, load_config, save_config
    ...
    from gabru_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `gabru model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from gabru_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from gabru_cli.config import save_env_value, load_config, save_config
    from gabru_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from gabru_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from gabru_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Gabru turns to `copilot --acp`.")
    print("  Gabru currently starts its own ACP subprocess for each request.")
    print("  Gabru uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set GABRU_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so gabru knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "ollama-cloud":
        from gabru_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from gabru_constants import display_gabru_home as _dhh_fn

            print(
                f"    Gabru will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from gabru_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'gabru model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Gabru CLI with a provider."""
    from gabru_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from gabru_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from gabru_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from gabru_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from gabru_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from gabru_cli.webhook import webhook_command
...
def cmd_hooks(args):
    ...
    from gabru_cli.hooks import hooks_command
    hooks_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from gabru_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from gabru_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from gabru_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from gabru_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Gabru home directory to a zip file."""
    if getattr(args, "quick", False):
        from gabru_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from gabru_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Gabru backup from a zip file."""
    from gabru_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Gabru Agent v{__version__} ({__release_date__})")
    ...
    try:
        from gabru_cli.banner import check_for_updates
        from gabru_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Gabru Agent."""
    _require_tty("uninstall")
    from gabru_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``gabru update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``gabru web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (gabru web will not be available)")
        )
        if fatal:
...
def _update_via_zip(args):
    """Update Gabru Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/gabru-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="gabru-update-")
        zip_path = os.path.join(tmp_dir, f"gabru-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to gabru-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"gabru-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "gabru-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Gabru behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes gabru completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Gabru couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Gabru couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Gabru behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `gabru update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/gabru-agent.git",
    "git@github.com:gabru-agent.git",
    "https://github.com/gabru-agent",
    "git@github.com:gabru-agent",
}
OFFICIAL_REPO_URL = "https://github.com/gabru-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_gabru_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_gabru_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Gabru repository.")
        print("  This means you may miss updates from gabru-agent.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/gabru-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/gabru-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``gabru update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from gabru_constants import get_default_gabru_root

    default_home = get_default_gabru_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "gabru-agent[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``gabru update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.gabru/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``gabru update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``gabru update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.gabru/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``gabru update --gateway``) the update is already
    ...
    try:
        ...
        # gabru_cli.config.get_gabru_home to simulate setup failure.
        from gabru_cli.config import get_gabru_home as _get_gabru_home

        logs_dir = _get_gabru_home() / "logs"
...
def cmd_update(args):
    """Update Gabru Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from gabru_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Gabru Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/gabru-agent/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_gabru_home added to gabru_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload gabru_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_gabru_home() added since the last release.
        try:
            import importlib
            import gabru_constants as _hc
        ...
        try:
            from gabru_cli.profiles import (
                list_profiles,
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print(
                    "    Run 'gabru config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: gabru config migrate")
            else:
                print()
                print("Skipped. Run 'gabru config migrate' later to configure.")
        else:
        ...
        # When running as ``gabru update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart gabru-gateway`` kills everything in the
        ...
        if gateway_mode:
            _exit_code_path = get_gabru_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all gabru-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. gabru-gateway.service or gabru-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: gabru gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print(
                            "    (or: gabru -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Gabru gateway unit files are still installed.
        # When both gabru.service (from a pre-rename install) and the
        # current gabru-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `gabru update` surfaces the issue until the user migrates.
        try:
            from gabru_cli.gateway import (
                has_legacy_gabru_units,
                _find_legacy_gabru_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_gabru_units():
                print()
                print("⚠ Legacy Gabru gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_gabru_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (gabru.service) fight the current")
                print("  gabru-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    gabru gateway migrate-legacy")
                print()
        ...
        print("  gabru model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``gabru -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `gabru profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_gabru_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → gabru -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.gabru)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  gabru profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     gabru -p {name} chat")
                else:
        ...
        from gabru_cli.profiles import profile_exists
        ...
        from gabru_cli.profiles import rename_profile
        ...
        from gabru_cli.profiles import export_profile
        ...
        from gabru_cli.profiles import import_profile
...
def cmd_dashboard(args):
    ...
    if "GABRU_WEB_DIST" not in os.environ:
        if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
            sys.exit(1)

    from gabru_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from gabru_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Gabru log files."""
    from gabru_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for gabru CLI."""
    parser = argparse.ArgumentParser(
        prog="gabru",
        description="Gabru Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    gabru                        Start interactive chat
    gabru chat -q "Hello"        Single query mode
    gabru -c                     Resume the most recent session
    gabru -c "my project"        Resume a session by name (latest in lineage)
    gabru --resume <session_id>  Resume a specific session by ID
    gabru setup                  Run setup wizard
    gabru logout                 Clear stored authentication
    gabru auth add <provider>    Add a pooled credential
    gabru auth list              List pooled credentials
    gabru auth remove <p> <t>    Remove pooled credential by index, id, or label
    gabru auth reset <provider>  Clear exhaustion status for a provider
    gabru model                  Select default model
    gabru config                 View configuration
    gabru config edit            Edit config in $EDITOR
    gabru config set model gpt-4 Set a config value
    gabru gateway                Run messaging gateway
    gabru -s gabru-agent-dev,github-auth
    gabru -w                     Start in isolated git worktree
    gabru gateway install        Install gateway background service
    gabru sessions list          List past sessions
    gabru sessions browse        Interactive session picker
    gabru sessions rename ID T   Rename/title a session
    gabru logs                   View agent.log (last 50 lines)
    gabru logs -f                Follow agent.log in real time
    gabru logs errors            View errors.log
    gabru logs --since 1h        Lines from the last hour
    gabru debug share             Upload debug report for support
    gabru update                 Update to latest version
...
""",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to GABRU_ACCEPT_HOOKS=1 or "
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Gabru Agent",
    )
    ...
    chat_parser.add_argument(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also GABRU_ACCEPT_HOOKS env var and "
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: gabru-cli)",
    )
    model_parser.add_argument(
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy gabru.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Gabru gateway unit files "
            "(e.g. gabru.service) left over from older installs. Profile "
            "units (gabru-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Gabru Agent with an interactive wizard. "
        "Run a specific section: gabru setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Gabru CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: gabru-cli)"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Gabru Agent components",
    )
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.gabru/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.gabru/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Gabru Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Gabru setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Gabru Agent. Use 'gabru debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    gabru debug share              Upload debug report and print URL
    gabru debug share --lines 500  Include more log lines
    gabru debug share --expire 30  Keep paste for 30 days
    gabru debug share --local      Print report locally (no upload)
    gabru debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'gabru debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Gabru home directory to a zip file",
        description="Create a zip archive of your entire Gabru configuration, "
        "skills, sessions, and data (excludes the gabru-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/gabru-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Gabru backup from a zip file",
        description="Extract a previously created Gabru backup into your "
        "Gabru home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Gabru Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from gabru_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.gabru/skills/.bundled_manifest) "
            "so future 'gabru update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from gabru_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from gabru_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/gabru-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `gabru plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from gabru_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_gabru_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_gabru_home()}/memories/\n")
        else:
            from gabru_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'gabru tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # gabru tools disable <name...> [--platform cli]
    ...
    # gabru tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from gabru_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from gabru_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Gabru as an MCP server",
        description=(
            "Manage MCP server connections and run Gabru as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'gabru mcp add' to connect to a new server, or\n"
            "'gabru mcp serve' to expose Gabru conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Gabru as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from gabru_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch gabru --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            gabru_bin = shutil.which("gabru")
            if gabru_bin:
                os.execvp(gabru_bin, ["gabru", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "gabru_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Gabru",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Gabru",
    ...
    def cmd_claw(args):
        from gabru_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Gabru Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Gabru Agent",
        description="Remove Gabru Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Gabru Agent as an ACP (Agent Client Protocol) server",
        description="Start Gabru Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Gabru Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Gabru instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Gabru Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Gabru log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    gabru logs                    Show last 50 lines of agent.log
    gabru logs -f                 Follow agent.log in real time
    gabru logs errors             Show last 50 lines of errors.log
    gabru logs gateway -n 100     Show last 100 lines of gateway.log
    gabru logs --level WARNING    Only show WARNING and above
    gabru logs --session abc123   Filter by session ID
    gabru logs --component tools  Only show tool-related lines
    gabru logs --since 1h         Lines from the last hour
    gabru logs --since 30m -f     Follow, starting from 30 min ago
    gabru logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``gabru -c Pokemon Agent Dev`` → ``gabru -c 'Pokemon Agent Dev'``
    ...
    from gabru_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'gabru -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (gabru hooks list, cron
    ...
    if (
        ...
        try:
            from gabru_cli.plugins import discover_plugins
            discover_plugins()
        except Exception:
            logger.debug(
