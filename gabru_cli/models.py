...
Add, remove, or reorder entries here — both `gabru setup` and
`gabru` provider-selection will pick up the change automatically.
"""
...
from gabru_cli import __version__ as _GABRU_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_GABRU_USER_AGENT = f"gabru-cli/{_GABRU_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `gabru model`
    flow without maintaining a separate static list.
    """
    from gabru_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    ...
    """
    base = (portal_base_url or "https://portal.").rstrip("/")
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from gabru_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  gabru model, /model, /provider, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `gabru model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `gabru model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``gabru model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``gabru auth add openai-codex`` without
    ``gabru model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    gabru-agent is tool-calling-first — every provider path assumes the model
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — gabru-agent
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from gabru_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return gabru-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; gabru's picker expects
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api./v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``gabru model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from gabru_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def detect_provider_for_model(
    ...
    if direct_match:
        ...
        if not has_creds:
            try:
                from gabru_cli.auth import _load_auth_store
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Gabru' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``gabru_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Gabru should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from gabru_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from gabru_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from gabru_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "GabruAgent/1.0",
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Gabru' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _GABRU_USER_AGENT}
    if api_key:
        headers["Authorization"] = f"Bearer {api_key}"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_gabru_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Gabru will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Gabru cannot verify the model name."
