"""
Single source of truth for provider identity in Gabru Agent.
...
2. **Gabru overlays** — transport type, auth patterns, aggregator flags,
...
# -- Gabru overlay ----------------------------------------------------------
# Gabru-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class GabruOverlay:
    """Gabru-specific provider metadata layered on top of models.dev."""
...
GABRU_OVERLAYS: Dict[str, GabruOverlay] = {
    "openrouter": GabruOverlay(
    ...
    "nous": GabruOverlay(
        ...
        base_url_override="https://inference-api./v1",
    ),
    "openai-codex": GabruOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": GabruOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="GABRU_QWEN_BASE_URL",
    ),
    "google-gemini-cli": GabruOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "copilot-acp": GabruOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": GabruOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": GabruOverlay(
    ...
    "kimi-for-coding": GabruOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": GabruOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": GabruOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": GabruOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": GabruOverlay(
        transport="openai_chat",
    ...
    "opencode": GabruOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": GabruOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "gabru", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Gabru overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Gabru overlay
    ...
    overlay = GABRU_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + gabru extra
    ...
    if overlay is not None:
        # Gabru-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in GABRU_OVERLAYS
    if provider == "bedrock":
