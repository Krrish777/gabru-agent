"""Centralized logging setup for Gabru Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.gabru/logs/`` (profile-aware via ``get_gabru_home()``).
...
from gabru_constants import get_config_path, get_gabru_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_gabru_session_injector", False):
    ...
    _session_record_factory._gabru_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``gabru logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_logging(
    *,
    ...
    """Configure the Gabru logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    gabru_home
        Override for the Gabru home directory.  Falls back to
        ``get_gabru_home()`` (profile-aware).
    log_level
    ...
    if _logging_initialized and not force:
        home = gabru_home or get_gabru_home()
        return home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_gabru_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._gabru_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the gabru group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from gabru_cli.config import is_managed
