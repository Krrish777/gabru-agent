"""
Gabru MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Gabru-specific extra)

Usage:
    gabru mcp serve
    gabru mcp serve --verbose
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using GABRU_HOME."""
    try:
        from gabru_constants import get_gabru_home
        return get_gabru_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("GABRU_HOME", Path.home() / ".gabru")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from gabru_constants import get_gabru_home
        directory_file = get_gabru_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("GABRU_HOME", Path.home() / ".gabru")
...
class EventBridge:
    ...
    This is the Gabru equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from gabru_constants import get_gabru_home
            db_file = get_gabru_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("GABRU_HOME", Path.home() / ".gabru")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Gabru MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "gabru",
        instructions=(
            "Gabru Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Gabru MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
