...
- smoke-test a server with the FastMCP CLI before wiring it into Gabru or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Gabru. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.gabru/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.gabru/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Gabru integration, either:

- configure the server in `~/.gabru/config.yaml` using the `native-mcp` skill, or
...
### Gabru cannot see the deployed server

The server-building part may be correct while the Gabru config is not. Load the `native-mcp` skill and configure the server in `~/.gabru/config.yaml`, then restart Gabru.
