...
# Configuration
REPO_URL_SSH="git@github.com:gabru-agent.git"
REPO_URL_HTTPS="https://github.com/gabru-agent.git"
GABRU_HOME="${GABRU_HOME:-$HOME/.gabru}"
INSTALL_DIR="${GABRU_INSTALL_DIR:-$GABRU_HOME/gabru-agent}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.gabru/gabru-agent)"
            echo "  --gabru-home PATH  Data directory (default: ~/.gabru, or \$GABRU_HOME)"
...
print_banner() {
    ...
    echo "│             ⚕ Gabru Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by .              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
get_gabru_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/gabru" ]; then
        echo "$link_dir/gabru"
    else
        echo "gabru"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/gabru-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$GABRU_HOME/node/bin/node" ]; then
        export PATH="$GABRU_HOME/node/bin:$PATH"
        local found_ver=$("$GABRU_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Gabru-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.gabru/node/..."
    ...
    # Place into ~/.gabru/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$GABRU_HOME/node"
    mkdir -p "$GABRU_HOME"
    mv "$extracted_dir" "$GABRU_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$GABRU_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$GABRU_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$GABRU_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$GABRU_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$GABRU_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.gabru/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Gabru Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="gabru-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Gabru behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up gabru command..."

    if [ "$USE_VENV" = true ]; then
        GABRU_BIN="$INSTALL_DIR/venv/bin/gabru"
    else
        GABRU_BIN="$(which gabru 2>/dev/null || echo "")"
        if [ -z "$GABRU_BIN" ]; then
            log_warn "gabru not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$GABRU_BIN" ]; then
        log_warn "gabru entry point not found at $GABRU_BIN"
    ...
    # Create a user-facing shim for the gabru command.
    mkdir -p "$command_link_dir"
    ln -sf "$GABRU_BIN" "$command_link_dir/gabru"
    log_success "Symlinked gabru → $command_link_display_dir/gabru"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "gabru command ready"
        return 0
    fi
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Gabru Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Gabru Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so gabru works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.gabru directory structure (config at top level, code in subdir)
    mkdir -p "$GABRU_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.gabru/.env (top level, easy to find)
    if [ ! -f "$GABRU_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$GABRU_HOME/.env"
            log_success "Created ~/.gabru/.env from template"
        else
            touch "$GABRU_HOME/.env"
            log_success "Created ~/.gabru/.env"
        fi
    else
        log_info "~/.gabru/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.gabru/config.yaml (top level, easy to find)
    if [ ! -f "$GABRU_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$GABRU_HOME/config.yaml"
            log_success "Created ~/.gabru/config.yaml from template"
        fi
    else
        log_info "~/.gabru/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$GABRU_HOME/SOUL.md" ]; then
        cat > "$GABRU_HOME/SOUL.md" << 'SOUL_EOF'
# Gabru Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Gabru communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.gabru/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.gabru/"

    # Seed bundled skills into ~/.gabru/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.gabru/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.gabru/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$GABRU_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$GABRU_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.gabru/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Gabru itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (gabru --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'gabru setup' after install."
        return 0
    fi
    ...
    # Run gabru setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m gabru_cli.main setup < /dev/tty
    else
        python -m gabru_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$GABRU_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Gabru to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$GABRU_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'gabru whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                GABRU_CMD="$(get_gabru_command_path)"
                $GABRU_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'gabru whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'gabru gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $GABRU_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $GABRU_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: gabru gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: gabru gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $GABRU_CMD gateway > "$GABRU_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.gabru/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: gabru gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: gabru gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.gabru/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.gabru/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.gabru/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.gabru/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.gabru/gabru-agent/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}gabru${NC}              Start chatting"
    echo -e "   ${GREEN}gabru setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}gabru config${NC}       View/edit configuration"
    echo -e "   ${GREEN}gabru config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}gabru gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}gabru update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'gabru' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'gabru' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
