#!/usr/bin/env bash
# Canonical test runner for gabru-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.gabru/gabru-agent/venv"; do
...
# Unset GABRU_* behavioral vars too.
unset GABRU_YOLO_MODE GABRU_INTERACTIVE GABRU_QUIET GABRU_TOOL_PROGRESS \
      GABRU_TOOL_PROGRESS_MODE GABRU_MAX_ITERATIONS GABRU_SESSION_PLATFORM \
      GABRU_SESSION_CHAT_ID GABRU_SESSION_CHAT_NAME GABRU_SESSION_THREAD_ID \
      GABRU_SESSION_SOURCE GABRU_SESSION_KEY GABRU_GATEWAY_SESSION \
      GABRU_PLATFORM GABRU_INFERENCE_PROVIDER GABRU_MANAGED GABRU_DEV \
      GABRU_CONTAINER GABRU_EPHEMERAL_SYSTEM_PROMPT GABRU_TIMEZONE \
      GABRU_REDACT_SECRETS GABRU_BACKGROUND_NOTIFICATIONS GABRU_EXEC_ASK \
      GABRU_HOME_MODE 2>/dev/null || true
