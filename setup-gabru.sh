...
# 5. Symlinks the 'gabru' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Gabru Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink gabru into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up gabru command..."

GABRU_BIN="$SCRIPT_DIR/venv/bin/gabru"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$GABRU_BIN" "$COMMAND_LINK_DIR/gabru"
echo -e "${GREEN}✓${NC} Symlinked gabru → $COMMAND_LINK_DISPLAY_DIR/gabru"
...
# ============================================================================
# Seed bundled skills into ~/.gabru/skills/
# ============================================================================

GABRU_SKILLS_DIR="${GABRU_HOME:-$HOME/.gabru}/skills"
mkdir -p "$GABRU_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.gabru/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$GABRU_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  gabru status        # Check configuration"
if is_termux; then
    echo "  gabru gateway       # Run gateway in foreground"
else
    echo "  gabru gateway install # Install gateway service (messaging + cron)"
fi
echo "  gabru cron list     # View scheduled jobs"
echo "  gabru doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m gabru_cli.main setup
fi
