...
"""
    ...
    def test_gateway_still_loads_skill_but_returns_setup_guidance(
        ...
        with patch.dict(
            os.environ, {"GABRU_SESSION_PLATFORM": "telegram"}, clear=False
        ):
            with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
                _make_skill(
                    tmp_path,
...
class TestPlanSkillHelpers:
    def test_build_plan_path_uses_workspace_relative_dir_and_slugifies_request(self):
        ...
        assert path == Path(".gabru") / "plans" / "2026-03-15_093045-implement-oauth-login-refresh-tokens.md"
    ...
    def test_plan_skill_message_can_include_runtime_save_path_note(self, tmp_path):
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            _make_skill(
                tmp_path,
                "plan",
                body="Save plans under .gabru/plans in the active workspace and do not execute the work.",
            )
            scan_skill_commands()
            msg = build_skill_invocation_message(
                ...
                runtime_note=(
                    ...
                    "the active workspace/backend cwd: .gabru/plans/plan.md"
                ),
            )

        assert msg is not None
        assert "Save plans under $GABRU_HOME/plans" not in msg
        assert ".gabru/plans" in msg
        assert "Add a /plan command" in msg
        assert ".gabru/plans/plan.md" in msg
...
class TestTemplateVarSubstitution:
    """``${GABRU_SKILL_DIR}`` and ``${GABRU_SESSION_ID}`` in SKILL.md body
    ...
    def test_substitutes_skill_dir(self, tmp_path):
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            skill_dir = _make_skill(
                tmp_path,
                "templated",
                body="Run: node ${GABRU_SKILL_DIR}/scripts/foo.js",
            )
            scan_skill_commands()
        ...
        assert "${GABRU_SKILL_DIR}" not in msg.split("[Skill directory:")[0]
    ...
    def test_leaves_session_id_token_when_missing(self, tmp_path):
        ...
        assert "Session: ${GABRU_SESSION_ID}" in msg
    ...
    def test_disable_template_vars_via_config(self, tmp_path):
        ...
        assert "${GABRU_SKILL_DIR}/scripts/foo.js" in msg
