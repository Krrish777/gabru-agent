...
def session_db(tmp_path):
    ...
    os.environ["GABRU_HOME"] = str(tmp_path / ".gabru")
    os.makedirs(tmp_path / ".gabru", exist_ok=True)
    from gabru_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".gabru" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal GabruCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        GabruCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        GabruCLI._handle_branch_command(cli_instance, "/branch refactor approach")
    ...
    def test_fork_alias(self):
        ...
        from gabru_cli.commands import resolve_command
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from gabru_cli.commands import COMMAND_REGISTRY
