...
def _isolate(tmp_path, monkeypatch):
    """Isolate GABRU_HOME so tests don't touch real config."""
    home = tmp_path / ".gabru"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal GabruCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = GabruCLI.__new__(GabruCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["gabru-core"]
        obj.compact = False
