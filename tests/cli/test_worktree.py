...
def _setup_worktree(repo_root):
    ...
    wt_name = f"gabru-{short_id}"
    branch_name = f"gabru/{wt_name}"
...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("gabru/gabru-")
...
class TestStaleWorktreePruning:
    ...
    def test_prunes_old_clean_worktree(self, git_repo):
        ...
        for entry in worktrees_dir.iterdir():
            if not entry.is_dir() or not entry.name.startswith("gabru-"):
                continue
            try:
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned gabru/* and pr-* branches."""

    def test_prunes_orphaned_gabru_branch(self, git_repo):
        """gabru/gabru-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "gabru/gabru-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "gabru/gabru-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "gabru/gabru-deadbeef" in result.stdout
        ...
        orphaned = [
            ...
            and (b.startswith("gabru/gabru-") or b.startswith("pr-"))
        ]
        assert "gabru/gabru-deadbeef" in orphaned
        ...
        assert "gabru/gabru-deadbeef" not in result.stdout
