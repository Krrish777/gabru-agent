"""Shared fixtures for the gabru-agent test suite.
...
2. **Isolated GABRU_HOME.** GABRU_HOME points to a per-test tempdir so
   code reading ``~/.gabru/*`` via ``get_gabru_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".gabru"`` instead of the canonical
   ``get_gabru_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No GABRU_SESSION_* inheritance** — the agent's current gateway
...
# GABRU_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_GABRU_BEHAVIORAL_VARS = frozenset({
    "GABRU_YOLO_MODE",
    "GABRU_INTERACTIVE",
    "GABRU_QUIET",
    "GABRU_TOOL_PROGRESS",
    ...
    "GABRU_SESSION_PLATFORM",
    "GABRU_SESSION_CHAT_ID",
    "GABRU_SESSION_CHAT_NAME",
    "GABRU_SESSION_THREAD_ID",
    "GABRU_SESSION_SOURCE",
    "GABRU_SESSION_KEY",
    "GABRU_GATEWAY_SESSION",
    "GABRU_PLATFORM",
    "GABRU_INFERENCE_PROVIDER",
    "GABRU_MANAGED",
    "GABRU_DEV",
    "GABRU_CONTAINER",
    "GABRU_EPHEMERAL_SYSTEM_PROMPT",
    "GABRU_TIMEZONE",
    "GABRU_REDACT_SECRETS",
    "GABRU_BACKGROUND_NOTIFICATIONS",
    "GABRU_EXEC_ASK",
    "GABRU_HOME_MODE",
    "BROWSER_CDP_URL",
    "CAMOFOX_URL",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and GABRU_HOME to per-test tempdirs so code that
    reads ``~/.gabru/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral GABRU_* vars that could change test semantics.
    for name in _GABRU_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect GABRU_HOME to a per-test tempdir. Code that reads
    #    ``~/.gabru/*`` via ``get_gabru_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.gabru/*`` via
    #    ``Path.home() / ".gabru"`` instead of ``get_gabru_home()``
    #    is a bug to fix at the callsite.
    fake_gabru_home = tmp_path / "gabru_test"
    fake_gabru_home.mkdir()
    (fake_gabru_home / "sessions").mkdir()
    (fake_gabru_home / "cron").mkdir()
    (fake_gabru_home / "memories").mkdir()
    (fake_gabru_home / "skills").mkdir()
    monkeypatch.setenv("GABRU_HOME", str(fake_gabru_home))
    ...
    #    ~/.gabru/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import gabru_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_gabru_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal gabru config dict suitable for unit tests."""
    return {
