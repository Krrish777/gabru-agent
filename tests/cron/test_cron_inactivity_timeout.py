...
- Unlimited timeout (GABRU_CRON_TIMEOUT=0)
- Backward compat: GABRU_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """GABRU_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """GABRU_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("GABRU_CRON_TIMEOUT", "1200")
        _cron_timeout = float(os.getenv("GABRU_CRON_TIMEOUT", 600))
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """GABRU_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("GABRU_CRON_TIMEOUT", "0")
        _cron_timeout = float(os.getenv("GABRU_CRON_TIMEOUT", 600))
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
...
class TestSysPathOrdering:
    ...
    def test_gabru_time_importable(self):
        """gabru_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _gabru_now
        assert callable(_gabru_now)

    def test_gabru_constants_importable(self):
        """gabru_constants should be importable from cron context."""
        from gabru_constants import get_gabru_home
        assert callable(get_gabru_home)
