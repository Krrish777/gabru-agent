...
- Path resolution (absolute, relative to GABRU_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp GABRU_HOME."""
    gabru_home = tmp_path / ".gabru"
    gabru_home.mkdir()
    (gabru_home / "cron").mkdir()
    (gabru_home / "cron" / "output").mkdir()
    (gabru_home / "scripts").mkdir()
    monkeypatch.setenv("GABRU_HOME", str(gabru_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "GABRU_DIR", gabru_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", gabru_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", gabru_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", gabru_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.gabru/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("GABRU_SESSION_PLATFORM") is None
        assert os.environ.get("GABRU_SESSION_CHAT_ID") is None
        assert os.environ.get("GABRU_SESSION_CHAT_NAME") is None
