...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._gabru_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("dotenv.load_dotenv"), \
             patch("gabru_state.SessionDB", return_value=fake_db), \
             patch(
                 "gabru_cli.runtime_provider.resolve_runtime_provider",
                 return_value={
                     "api_key": "test-key",
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("TELEGRAM_HOME_CHANNEL", raising=False)
        monkeypatch.delenv("GABRU_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("GABRU_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("GABRU_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("GABRU_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("GABRU_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("GABRU_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("GABRU_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("GABRU_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("GABRU_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
...
class TestRunJobSkillBacked:
    ...
    def test_run_job_preserves_credential_file_passthrough_into_worker_thread(self, tmp_path):
        ...
        with patch("cron.scheduler._gabru_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("tools.credential_files._resolve_gabru_home", return_value=tmp_path), \
             patch("dotenv.load_dotenv"), \
             patch("gabru_state.SessionDB", return_value=fake_db), \
             patch(
                 "gabru_cli.runtime_provider.resolve_runtime_provider",
...
class TestParallelTick:
    ...
    def test_parallel_jobs_isolated_contextvars(self):
        ...
        def mock_run_job(job):
            ...
            platform = get_session_env("GABRU_SESSION_PLATFORM")
            chat_id = get_session_env("GABRU_SESSION_CHAT_ID")
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """GABRU_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("GABRU_CRON_MAX_PARALLEL", "1")
        call_times = []
