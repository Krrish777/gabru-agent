...
subparser, placing the flag BEFORE the subcommand (e.g. 'gabru --yolo chat')
...
def _build_parser():
    """Build the gabru argument parser from the real code.
    ...
    """
    parser = argparse.ArgumentParser(prog="gabru")
...
class TestYoloEnvVar:
    """Verify --yolo sets GABRU_YOLO_MODE regardless of flag position.
    ...
    def _clean_env(self):
        os.environ.pop("GABRU_YOLO_MODE", None)
        yield
        os.environ.pop("GABRU_YOLO_MODE", None)
    ...
    def test_yolo_before_chat_sets_env(self):
        ...
        assert os.environ.get("GABRU_YOLO_MODE") == "1"
    ...
    def test_no_yolo_no_env(self):
        ...
        self._simulate_cmd_chat_yolo_check(args)
        assert os.environ.get("GABRU_YOLO_MODE") is None
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `gabru gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `gabru <argv>` must exit 0 (help) rather than
        failing with `unrecognized arguments`."""
        import subprocess
        result = subprocess.run(
            [sys.executable, "-m", "gabru_cli.main", *argv],
            capture_output=True,
            text=True,
            timeout=15,
        )
