...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    gabru_home.mkdir(parents=True, exist_ok=True)
    (gabru_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "gabru" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_gabru_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:gabru_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:gabru_pkce"
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "gabru_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `gabru auth add nous` must also populate providers.nous so the
    ...
    # raises "Gabru is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`gabru auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "gabru_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from gabru_cli.auth_commands import auth_reset_command
...
def test_clear_provider_auth_removes_provider_pool_entries(tmp_path, monkeypatch):
    ...
    from gabru_cli.auth import clear_provider_auth
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from gabru_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "gabru_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("gabru_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("gabru_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("gabru_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "gabru_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    updated = json.loads((gabru_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from gabru_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `gabru auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("GABRU_HOME", str(tmp_path / "gabru"))
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    ...
    # Pre-existing suppression (simulating a prior `gabru auth remove`)
    (gabru_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    ...
    payload = json.loads((gabru_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("gabru_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((gabru_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`gabru auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.gabru/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.gabru/.env (not the shell), the
    ...
    (gabru_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (gabru_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `gabru auth add <provider>` clears any
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `gabru auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Gabru reads from has a
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import gabru_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_gabru_pkce_suppression(tmp_path, monkeypatch):
    """anthropic gabru_pkce must not re-seed from ~/.gabru/.anthropic_oauth.json when suppressed."""
    gabru_home = tmp_path / "gabru"
    gabru_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("GABRU_HOME", str(gabru_home))

    import yaml
    (gabru_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (gabru_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["gabru_pkce"]},
    }))

    # Stub the readers so only gabru_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_gabru_oauth_credentials", lambda: {
    ...
    # gabru_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "gabru_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (gabru_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `gabru auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `gabru auth add openai-codex`)
