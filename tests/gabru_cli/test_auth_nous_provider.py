...
from gabru_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from gabru_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("GABRU_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_gabru_ca_bundle_env_falls_back(self, monkeypatch):
        from gabru_cli.auth import _resolve_verify

        monkeypatch.setenv("GABRU_CA_BUNDLE", "/nonexistent/gabru-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    (gabru_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from gabru_cli.auth import get_nous_auth_status
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    gabru_home = tmp_path / "gabru"
    _setup_nous_auth(gabru_home, access_token="at-123")
    monkeypatch.setenv("GABRU_HOME", str(gabru_home))
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    gabru_home = tmp_path / "gabru"
    _setup_nous_auth(gabru_home, refresh_token="refresh-old")
    monkeypatch.setenv("GABRU_HOME", str(gabru_home))
    ...
    monkeypatch.setattr("gabru_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("gabru_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `gabru model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        auth_path = gabru_home / "auth.json"
        auth_path.write_text(json.dumps({
            "version": 1,
            "active_provider": "openrouter",
            "providers": {"openrouter": {"api_key": "sk-or-fake"}},
        }))
        return gabru_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import gabru_cli.models as models_mod
        import gabru_cli.nous_subscription as ns

        fake_auth_state = {
            "access_token": "fake-nous-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.",
            "portal_base_url": "https://portal.",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from gabru_cli.auth import PROVIDER_REGISTRY, _login_nous

        gabru_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `gabru auth add nous`
    ...
    """
    from gabru_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Gabru is not logged into Nous Portal").
    ...
    """
    from gabru_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
    ...
    # doesn't raise "Gabru is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `gabru auth add nous --type oauth --label <name>` silently
...
def test_persist_nous_credentials_custom_label_survives_reseed(tmp_path, monkeypatch):
    ...
    """
    from gabru_cli.auth import persist_nous_credentials
