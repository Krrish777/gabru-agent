"""Tests for the auxiliary-model configuration UI in ``gabru model``.
...
from gabru_cli.config import DEFAULT_CONFIG, load_config
...
def test_save_aux_choice_persists_to_config_yaml(tmp_path, monkeypatch):
    ...
    (tmp_path / ".gabru").mkdir(exist_ok=True)
