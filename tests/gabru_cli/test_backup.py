"""Tests for gabru backup and import commands."""
...
def _make_gabru_tree(root: Path) -> None:
    """Create a realistic ~/.gabru directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "gabru_state.db").write_bytes(b"fake-state")
    ...
    # gabru-agent repo (should be EXCLUDED)
    (root / "gabru-agent").mkdir(exist_ok=True)
    (root / "gabru-agent" / "run_agent.py").write_text("# big file\n")
    (root / "gabru-agent" / ".git").mkdir()
    (root / "gabru-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_gabru_agent(self):
        from gabru_cli.backup import _should_exclude
        assert _should_exclude(Path("gabru-agent/run_agent.py"))
        assert _should_exclude(Path("gabru-agent/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        gabru_home.mkdir()
        _make_gabru_tree(gabru_home)

        monkeypatch.setenv("GABRU_HOME", str(gabru_home))
        # get_default_gabru_root needs this
    ...
    def test_excludes_gabru_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include gabru-agent/ directory."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        _make_gabru_tree(gabru_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "gabru-agent" in n]
            assert agent_files == [], f"gabru-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/gabru-backup-*.zip."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        (gabru_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("gabru-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Gabru backup."""
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only gabru_state.db (old wrong name) is rejected."""
        from gabru_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["gabru_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into gabru home."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (gabru_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (gabru_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (gabru_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (gabru_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_gabru_prefix(self, tmp_path, monkeypatch):
        """Import strips .gabru/ prefix if all entries share it."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".gabru/config.yaml": "model: test\n",
            ".gabru/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (gabru_home / "config.yaml").read_text() == "model: test\n"
        assert (gabru_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_gabru_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a gabru backup."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (gabru_home / "config.yaml").exists()
        # traversal file should NOT exist outside gabru home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (gabru_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (gabru_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (gabru_home / "config.yaml").read_text() == "model: restored\n"
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".gabru"
        src_home.mkdir(parents=True)
        _make_gabru_tree(src_home)

        monkeypatch.setenv("GABRU_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from gabru_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".gabru"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("GABRU_HOME", str(dst_home))
        ...
        # gabru-agent should NOT be present
        assert not (dst_home / "gabru-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from gabru_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_gabru(self):
        ...
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".gabru/config.yaml", "test")
            zf.writestr(".gabru/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".gabru/"
...
class TestBackupEdgeCases:
    def test_nonexistent_gabru_home(self, tmp_path, monkeypatch):
        """Backup exits when gabru home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".gabru"
        monkeypatch.setenv("GABRU_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("gabru-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_gabru_home(self, tmp_path, monkeypatch):
        """Backup handles empty gabru home (no files to back up)."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        # Only excluded dirs, no actual files
        (gabru_home / "__pycache__").mkdir()
        (gabru_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = gabru_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_gabru(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside gabru root."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        (gabru_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside gabru home
        out_zip = gabru_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        (gabru_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        (gabru_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (gabru_home / "config.yaml").exists()
        assert (gabru_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (gabru_home / "profiles" / "coder" / "config.yaml").exists()
        assert (gabru_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "gabru -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from gabru_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def gabru_home(self, tmp_path):
        """Create a fake GABRU_HOME with critical state files."""
        home = tmp_path / ".gabru"
        home.mkdir()
    ...
    def test_creates_snapshot(self, gabru_home):
        from gabru_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(gabru_home=gabru_home)
        assert snap_id is not None
        snap_dir = gabru_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, gabru_home):
        from gabru_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", gabru_home=gabru_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, gabru_home):
        from gabru_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(gabru_home=gabru_home)
        db_copy = gabru_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, gabru_home):
        from gabru_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(gabru_home=gabru_home)
        assert (gabru_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, gabru_home):
        from gabru_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(gabru_home=gabru_home)
        with open(gabru_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(gabru_home=empty) is None

    def test_list_snapshots(self, gabru_home):
        from gabru_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", gabru_home=gabru_home)
        id2 = create_quick_snapshot(label="second", gabru_home=gabru_home)

        snaps = list_quick_snapshots(gabru_home=gabru_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, gabru_home):
        from gabru_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", gabru_home=gabru_home)
        snaps = list_quick_snapshots(limit=3, gabru_home=gabru_home)
        assert len(snaps) == 3

    def test_restore_config(self, gabru_home):
        from gabru_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(gabru_home=gabru_home)

        (gabru_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (gabru_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, gabru_home=gabru_home)
        assert result is True
        assert "openrouter" in (gabru_home / "config.yaml").read_text()

    def test_restore_state_db(self, gabru_home):
        from gabru_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(gabru_home=gabru_home)

        conn = sqlite3.connect(str(gabru_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, gabru_home=gabru_home)
    ...
    def test_restore_nonexistent(self, gabru_home):
        from gabru_cli.backup import restore_quick_snapshot
    ...
    def test_manual_prune(self, gabru_home):
        ...
        assert deleted == 7
        assert len(list_quick_snapshots(gabru_home=gabru_home)) == 3
