...
Covers:
 - Normal path (tokens already in Gabru auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Gabru
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `gabru auth openai-codex`. The old
...
@pytest.fixture()
def gabru_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Gabru auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(gabru_auth_only_env):
    """openai-codex appears when tokens are already in Gabru auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Gabru
    auth store."""
    gabru_home = tmp_path / ".gabru"
    gabru_home.mkdir()
    ...
    (gabru_home / "auth.json").write_text(
