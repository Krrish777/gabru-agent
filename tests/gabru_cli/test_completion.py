"""Tests for gabru_cli/completion.py — shell completion script generation."""
...
from gabru_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real gabru structure."""
    p = argparse.ArgumentParser(prog="gabru")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_gabru_completion()" in out
        assert "complete -F _gabru_completion gabru" in out
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c gabru -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from gabru_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_gabru_profiles()" in out
        assert 'profiles_dir="$HOME/.gabru/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'gabru profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _gabru_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_gabru_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _gabru_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_gabru_profiles()" in out
        assert "$HOME/.gabru/profiles" in out
    ...
    def test_fish_has_profiles_helper(self):
        out = generate_fish(_make_parser())
        assert "__gabru_profiles" in out
        assert "$HOME/.gabru/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__gabru_profiles" in out
        count = out.count("(__gabru_profiles)")
