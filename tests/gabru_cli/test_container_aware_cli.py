...
def container_env(tmp_path, monkeypatch):
    """Set up a fake GABRU_HOME with .container-mode file."""
    gabru_home = tmp_path / ".gabru"
    gabru_home.mkdir()
    monkeypatch.setenv("GABRU_HOME", str(gabru_home))
    monkeypatch.delenv("GABRU_DEV", raising=False)

    container_mode = gabru_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=gabru-agent\n"
        "exec_user=gabru\n"
        "gabru_bin=/data/current-package/bin/gabru\n"
    )
    return gabru_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("gabru_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "gabru-agent"
    assert info["exec_user"] == "gabru"
    assert info["gabru_bin"] == "/data/current-package/bin/gabru"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("gabru_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_gabru_dev(container_env, monkeypatch):
    """Returns None when GABRU_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("GABRU_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_gabru_dev_zero(container_env, monkeypatch):
    """GABRU_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("GABRU_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        gabru_home = Path(tmpdir) / ".gabru"
        gabru_home.mkdir()
        (gabru_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("gabru_constants.is_container", return_value=False), \
             patch("gabru_cli.config.get_gabru_home", return_value=gabru_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("GABRU_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=gabru-custom\n"
        "exec_user=myuser\n"
        "gabru_bin=/opt/gabru/bin/gabru\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "gabru-custom"
    assert info["exec_user"] == "myuser"
    assert info["gabru_bin"] == "/opt/gabru/bin/gabru"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from gabru_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "gabru"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "gabru-agent" in cmd
    assert "/data/current-package/bin/gabru" in cmd
    assert "chat" in cmd
