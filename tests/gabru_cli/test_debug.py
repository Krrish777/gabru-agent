...
def gabru_home(tmp_path, monkeypatch):
    """Set up an isolated GABRU_HOME with minimal logs."""
    home = tmp_path / ".gabru"
    home.mkdir()
    monkeypatch.setenv("GABRU_HOME", str(home))
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from gabru_cli.debug import _upload_paste_rs
        ...
        with patch("gabru_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "gabru_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from gabru_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from gabru_cli.debug import upload_to_pastebin

        with patch("gabru_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("gabru_cli.debug._upload_paste_rs",
             ...
             patch("gabru_cli.debug._upload_dpaste_com",
...
class TestReadFullLog:
    ...
    def test_reads_small_file(self, gabru_home):
        from gabru_cli.debug import _read_full_log
    ...
    def test_returns_none_for_empty(self, gabru_home):
        # Truncate agent.log to empty
        (gabru_home / "logs" / "agent.log").write_text("")

        from gabru_cli.debug import _read_full_log
        assert _read_full_log("agent") is None

    def test_truncates_large_file(self, gabru_home):
        """Files larger than max_bytes get tail-truncated."""
        from gabru_cli.debug import _read_full_log

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (gabru_home / "logs" / "agent.log").write_text(big_content * 200)

        content = _read_full_log("agent", max_bytes=1024)
        assert content is not None
        assert "truncated" in content

    def test_unknown_log_returns_none(self, gabru_home):
        from gabru_cli.debug import _read_full_log
        assert _read_full_log("nonexistent") is None

    def test_falls_back_to_rotated_file(self, gabru_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from gabru_cli.debug import _read_full_log

        logs_dir = gabru_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, gabru_home):
    ...
    def test_falls_back_when_primary_empty(self, gabru_home):
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, gabru_home):
        from gabru_cli.debug import collect_debug_report

        with patch("gabru_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- gabru dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- gabru dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, gabru_home):
        from gabru_cli.debug import collect_debug_report

        with patch("gabru_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, gabru_home):
        from gabru_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, gabru_home):
        from gabru_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_local_flag_prints_full_logs(self, gabru_home, capsys):
        """--local prints the report plus full log contents."""
        from gabru_cli.debug import run_debug_share
    ...
    def test_share_uploads_three_pastes(self, gabru_home, capsys):
        ...
        with patch("gabru_cli.dump.run_dump") as mock_dump, \
             patch("gabru_cli.debug.upload_to_pastebin",
                    side_effect=_mock_upload):
            mock_dump.side_effect = lambda a: print("--- gabru dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, gabru_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, gabru_home, capsys):
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "gabru debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, gabru_home):
        from gabru_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from gabru_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from gabru_cli.debug import delete_paste
        ...
        with patch("gabru_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.gabru/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``gabru debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, gabru_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, gabru_home):
        from gabru_cli.debug import _sweep_expired_pastes
    ...
    def test_sweep_deletes_expired_entries(self, gabru_home):
        ...
        with patch("gabru_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, gabru_home):
        ...
        with patch("gabru_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, gabru_home):
        from gabru_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("gabru_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, gabru_home, capsys):
        ...
        with patch(
            "gabru_cli.debug._sweep_expired_pastes",
        ...
        out = capsys.readouterr().out
        assert "Usage: gabru debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from gabru_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("gabru_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("gabru_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, gabru_home, capsys):
        from gabru_cli.debug import run_debug_share
        ...
        with patch("gabru_cli.dump.run_dump"), \
             ...
             patch("gabru_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, gabru_home, capsys):
        from gabru_cli.debug import run_debug_share
        ...
        with patch("gabru_cli.dump.run_dump"), \
             ...
             patch("gabru_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, gabru_home, capsys):
        from gabru_cli.debug import run_debug_share
