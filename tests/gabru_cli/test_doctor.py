...
import gabru_cli.doctor as doctor
import gabru_cli.gateway as gateway_cli
from gabru_cli import doctor as doctor_mod
from gabru_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "GABRU_HOME", gabru_home)
    monkeypatch.delenv("GABRU_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("GABRU_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_gabru_home(self, tmp_path, provider=""):
        """Create a minimal GABRU_HOME with config.yaml."""
        home = tmp_path / ".gabru"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_gabru_home(tmp_path, provider)
