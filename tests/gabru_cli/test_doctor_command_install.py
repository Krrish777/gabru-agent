"""Tests for the Command Installation check in gabru doctor."""
...
import gabru_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal GABRU_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".gabru"
    ...
    gabru_bin = venv_bin_dir / "gabru"
    gabru_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    gabru_bin.chmod(0o755)

    monkeypatch.setattr(doctor_mod, "GABRU_HOME", home)
    ...
    # Stub auth checks
    try:
        from gabru_cli import auth as _auth_mod
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, gabru_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "gabru"
        cmd_link.symlink_to(gabru_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "gabru doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "gabru"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == gabru_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "gabru"
        wrong_target = tmp_path / "wrong_gabru"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        gabru_bin = project / ".venv" / "bin" / "gabru"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "gabru"
        cmd_link.symlink_to(gabru_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/gabru" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/gabru is a regular file (not symlink), accept it."""
        home, project, gabru_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "gabru"
        cmd_link.write_text("#!/bin/sh\nexec python -m gabru_cli.main \"$@\"\n")
