"""Tests for the top-level `./gabru` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./gabru` should use `gabru_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "gabru"
    called = []

    fake_main_module = types.ModuleType("gabru_cli.main")

    def fake_main():
        called.append("gabru_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "gabru_cli.main", fake_main_module)
