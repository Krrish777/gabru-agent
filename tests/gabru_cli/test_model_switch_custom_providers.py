...
The terminal `gabru model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import gabru_cli.providers as providers_mod
from gabru_cli.model_switch import list_authenticated_providers, switch_model
from gabru_cli.providers import resolve_provider_full
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "GABRU_OVERLAYS", {})
...
def test_switch_model_accepts_explicit_named_custom_provider(monkeypatch):
    ...
    monkeypatch.setattr("gabru_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("gabru_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("gabru_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: Gabru's own writer stores configured models as a dict
