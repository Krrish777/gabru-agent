...
``gabru_cli.runtime_provider.resolve_runtime_provider`` already strips
``/v1`` at fresh agent init (PR #4918), but the ``/model`` mid-session
switch path in ``gabru_cli.model_switch.switch_model`` was missing the
...
from gabru_cli.model_switch import switch_model
...
def _run_opencode_switch(
    ...
    with (
        patch("gabru_cli.model_switch.resolve_alias", return_value=None),
        patch("gabru_cli.model_switch.list_provider_models", return_value=[]),
        patch(
        ...
        patch(
            "gabru_cli.models.validate_requested_model",
            return_value=_MOCK_VALIDATION,
        ),
        patch("gabru_cli.model_switch.get_model_info", return_value=None),
        patch("gabru_cli.model_switch.get_model_capabilities", return_value=None),
        patch("gabru_cli.models.detect_provider_for_model", return_value=None),
    ):
        return switch_model(
            raw_input=raw_input,
