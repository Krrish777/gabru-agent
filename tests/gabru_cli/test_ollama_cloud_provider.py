...
from gabru_cli.auth import PROVIDER_REGISTRY, resolve_provider, resolve_api_key_provider_credentials
from gabru_cli.models import _PROVIDER_MODELS, _PROVIDER_LABELS, _PROVIDER_ALIASES, normalize_provider
...
class TestOllamaCloudModelCatalog:
    ...
    def test_provider_model_ids_returns_dynamic_models(self, tmp_path, monkeypatch):
        ...
        from gabru_cli.models import provider_model_ids
        ...
        with patch("gabru_cli.models.fetch_api_models", return_value=["qwen3.5:397b"]), \
...
class TestOllamaCloudMergedDiscovery:
    def test_merges_live_and_models_dev(self, tmp_path, monkeypatch):
        ...
        with patch("gabru_cli.models.fetch_api_models", return_value=["qwen3.5:397b", "glm-5"]), \
             patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev):
    ...
    def test_uses_disk_cache(self, tmp_path, monkeypatch):
        ...
        with patch("gabru_cli.models.fetch_api_models", return_value=["model-a"]) as mock_api, \
    ...
    def test_stale_cache_used_on_total_failure(self, tmp_path, monkeypatch):
        ...
        from gabru_cli.models import fetch_ollama_cloud_models, _save_ollama_cloud_cache
        ...
        with patch("gabru_cli.models.fetch_api_models", return_value=None), \
             patch("agent.models_dev.fetch_models_dev", return_value={}):
            result = fetch_ollama_cloud_models(force_refresh=True)
...
class TestOllamaCloudProvidersNew:
    def test_overlay_exists(self):
        from gabru_cli.providers import GABRU_OVERLAYS
        assert "ollama-cloud" in GABRU_OVERLAYS
        overlay = GABRU_OVERLAYS["ollama-cloud"]
        assert overlay.transport == "openai_chat"
        assert overlay.base_url_env_var == "OLLAMA_BASE_URL"

    def test_alias_resolves(self):
        from gabru_cli.providers import normalize_provider as np
        assert np("ollama") == "custom"  # bare "ollama" = local
        assert np("ollama-cloud") == "ollama-cloud"

    def test_label_override(self):
        from gabru_cli.providers import _LABEL_OVERRIDES
        assert _LABEL_OVERRIDES.get("ollama-cloud") == "Ollama Cloud"

    def test_get_label(self):
        from gabru_cli.providers import get_label
        assert get_label("ollama-cloud") == "Ollama Cloud"

    def test_get_provider(self):
        from gabru_cli.providers import get_provider
