"""Tests for the Gabru plugin system (gabru_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<gabru_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<gabru_home>/plugins/``; we derive
    ``<gabru_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <GABRU_HOME>/config.yaml.
        # Config is always read from GABRU_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        gabru_home_str = os.environ.get("GABRU_HOME")
        if gabru_home_str:
            gabru_home = Path(gabru_home_str)
        else:
            gabru_home = base.parent
        gabru_home.mkdir(parents=True, exist_ok=True)
        cfg_path = gabru_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.gabru/plugins/ are discovered."""
        plugins_dir = tmp_path / "gabru_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("GABRU_HOME", str(tmp_path / "gabru_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("GABRU_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".gabru" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        gabru_home = tmp_path / "gabru_test"
        (gabru_home / "config.yaml").write_text(
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under gabru_plugins.<name>."""
        plugins_dir = tmp_path / "gabru_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("GABRU_HOME", str(tmp_path / "gabru_test"))

        # Clean up any prior namespace module
        sys.modules.pop("gabru_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "gabru_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="gabru_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("gabru_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        gabru_home = tmp_path / "gabru_test"
        plugins_dir = gabru_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "gabru_test" / "plugins"
        # Set GABRU_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("gabru_cli.plugins.PluginContext.dispatch_tool.__module__", "gabru_cli.plugins"):
