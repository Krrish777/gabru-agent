"""Comprehensive tests for gabru_cli.profiles module.
...
from gabru_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_gabru_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and GABRU_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.gabru/profiles)
    * GABRU_HOME  -> tmp_path/.gabru  (so get_gabru_home() agrees)
    * Creates the bare-minimum ~/.gabru directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".gabru"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("GABRU_HOME", str(default_home))
    return tmp_path
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("gabru_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".gabru" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_gabru_home_returns_default(self, profile_env):
        # GABRU_HOME points to tmp_path/.gabru which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".gabru" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom GABRU_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("GABRU_HOME", str(custom))
        # With Docker-aware roots, a custom GABRU_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("gabru")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".gabru" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("gabru_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".gabru" / "profiles" / "newname"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("gabru-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".gabru_history",
        ...
        excluded_prefixes = [
            "default/gabru-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.gabru_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_gabru_profiles_function(self):
        script = generate_bash_completion()
        assert "_gabru_profiles" in script

    def test_zsh_completion_has_gabru_function(self):
        script = generate_zsh_completion()
        assert "_gabru" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultGabruHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_gabru_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".gabru" / "profiles"

    def test_default_gabru_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_gabru_home()
        assert home == tmp_path / ".gabru"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (GABRU_HOME outside ~/.gabru), profiles go under GABRU_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GABRU_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_gabru_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_gabru_home() returns GABRU_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GABRU_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (GABRU_HOME under ~/.gabru), profiles root is still ~/.gabru/profiles."""
        native = tmp_path / ".gabru"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under GABRU_HOME."""
        from gabru_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GABRU_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under GABRU_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GABRU_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GABRU_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".gabru" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from gabru_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".gabru"
