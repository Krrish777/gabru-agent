...
from gabru_cli.auth import get_active_provider
from gabru_cli.config import load_config, save_config
from gabru_cli import setup as setup_mod
from gabru_cli.setup import setup_model_provider
...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("gabru_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("gabru_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_delegates_to_select_provider_and_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("gabru_cli.main.select_provider_and_model", fake_select)
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import gabru_cli.gateway as gateway_mod
...
def test_setup_gateway_in_container_shows_docker_guidance(monkeypatch, capsys):
    ...
    monkeypatch.setattr(gabru_constants, "is_container", lambda: True)
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("gabru_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("gabru_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "gabru_cli.main._model_flow_named_custom",
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("named custom flow should not run")),
    )

    from gabru_cli.main import select_provider_and_model
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("gabru_cli.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("GABRU_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("gabru_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("gabru_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("gabru_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "gabru_cli.setup.get_nous_subscription_features",
    ...
    from gabru_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("gabru_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("gabru_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("gabru_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "gabru_cli.setup.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
...
def test_resolve_gabru_chat_argv_prefers_which(monkeypatch):
    from gabru_cli import setup as setup_mod

    monkeypatch.setattr(setup_mod.shutil, "which", lambda name: "/usr/local/bin/gabru" if name == "gabru" else None)

    assert setup_mod._resolve_gabru_chat_argv() == ["/usr/local/bin/gabru", "chat"]


def test_resolve_gabru_chat_argv_falls_back_to_module(monkeypatch):
    from gabru_cli import setup as setup_mod

    monkeypatch.setattr(setup_mod.shutil, "which", lambda _name: None)
    monkeypatch.setattr(setup_mod.importlib.util, "find_spec", lambda name: object() if name == "gabru_cli" else None)

    assert setup_mod._resolve_gabru_chat_argv() == [sys.executable, "-m", "gabru_cli.main", "chat"]
...
def test_offer_launch_chat_execs_fresh_process(monkeypatch):
    ...
    monkeypatch.setattr(setup_mod, "_resolve_gabru_chat_argv", lambda: ["/usr/local/bin/gabru", "chat"])
    ...
    assert exec_calls == [("/usr/local/bin/gabru", ["/usr/local/bin/gabru", "chat"])]
...
def test_offer_launch_chat_manual_fallback_when_unresolvable(monkeypatch, capsys):
    ...
    monkeypatch.setattr(setup_mod, "prompt_yes_no", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(setup_mod, "_resolve_gabru_chat_argv", lambda: None)

    setup_mod._offer_launch_chat()

    captured = capsys.readouterr()
    assert "Run 'gabru chat' manually" in captured.out
