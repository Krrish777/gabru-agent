...
import pytest
from gabru_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from gabru_cli.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("gabru_cli.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_cmd_setup_defers_no_tty_handling_to_setup_wizard(self):
        """Bare `gabru setup` should reach setup.py, which prints headless guidance."""
        from gabru_cli.main import cmd_setup
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("gabru_cli.setup.ensure_gabru_home"),
            patch("gabru_cli.setup.load_config", return_value={}),
            patch("gabru_cli.setup.get_gabru_home", return_value="/tmp/.gabru"),
            patch("gabru_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "gabru config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `gabru` should not prompt for input when no provider exists and stdin is headless."""
        from gabru_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("gabru_cli.main._has_any_provider_configured", return_value=False),
            patch("gabru_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
    ...
    def test_returning_user_terminal_menu_choice_dispatches_terminal_section(self, tmp_path):
        ...
        with (
            patch.object(setup_mod, "ensure_gabru_home"),
            patch.object(setup_mod, "load_config", return_value=config),
            patch.object(setup_mod, "get_gabru_home", return_value=tmp_path),
            ...
            patch("gabru_cli.auth.get_active_provider", return_value=None),
    ...
    def test_main_accepts_tts_setup_section(self, monkeypatch):
        """`gabru setup tts` should parse and dispatch like other setup sections."""
        from gabru_cli import main as main_mod
        ...
        monkeypatch.setattr("sys.argv", ["gabru", "setup", "tts"])
