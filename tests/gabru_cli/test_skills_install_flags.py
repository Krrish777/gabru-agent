"""
Tests for --yes / --force flag separation in `gabru skills install`.
...
def test_cli_skills_install_yes_sets_skip_confirm(monkeypatch):
    ...
    from gabru_cli.main import main
    ...
    monkeypatch.setattr("gabru_cli.skills_hub.skills_command", fake_skills_command)
    monkeypatch.setattr(
        sys,
        "argv",
        ["gabru", "skills", "install", "official/email/agentmail", "--yes"],
    )
...
def test_cli_skills_install_y_alias(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["gabru", "skills", "install", "test/skill", "-y"],
    )
...
def test_cli_skills_install_force_sets_force(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["gabru", "skills", "install", "test/skill", "--force"],
    )
...
def test_cli_skills_install_force_and_yes_together(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["gabru", "skills", "install", "test/skill", "--force", "--yes"],
    )
...
def test_cli_skills_install_no_flags(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["gabru", "skills", "install", "test/skill"],
    )
