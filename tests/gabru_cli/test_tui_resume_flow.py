...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    import gabru_cli.main as main_mod
    ...
    monkeypatch.setitem(sys.modules, "gabru_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB()))

    main_mod._print_tui_exit_summary("20260409_000001_abc123")
    out = capsys.readouterr().out

    assert "Resume this session with:" in out
    assert "gabru --tui --resume 20260409_000001_abc123" in out
    assert 'gabru --tui -c "demo title"' in out
