"""Tests for the update check mechanism in gabru_cli.banner."""
...
def test_version_string_no_v_prefix():
    ...
    from gabru_cli import __version__
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("gabru_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("GABRU_HOME", str(tmp_path))
    with patch("gabru_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import gabru_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "gabru_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when GABRU_HOME has no git repo."""
    import gabru_cli.banner as banner
    ...
    # Point GABRU_HOME at a temp dir with no gabru-agent/.git
    monkeypatch.setenv("GABRU_HOME", str(tmp_path))
    with patch("gabru_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from gabru_cli.main import _invalidate_update_cache

    # Build a fake ~/.gabru with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"GABRU_HOME": str(default_home)}):
