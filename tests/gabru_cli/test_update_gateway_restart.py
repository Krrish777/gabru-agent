...
Ensures ``gabru update`` correctly detects running gateways managed by
...
def _no_restart_verify_sleep(monkeypatch):
    """gabru_cli/main.py uses time.sleep(3) after systemctl restart to
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units gabru-gateway* — discover all gateway services
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="gabru-gateway.service loaded active running Gabru Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>GABRU_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: gabru gateway run'."""
        ...
        assert "Restart manually: gabru gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: gabru gateway run" in captured
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted gabru-gateway" in captured
...
class TestServicePidExclusion:
    ...
    where ``gabru update`` would restart the gateway and immediately kill it.
    """
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.gabru.gateway")
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="gabru-gateway.service loaded inactive dead Gabru Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.gabru/gabru-agent/venv/bin/python -m gabru_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.gabru/gabru-agent/venv/bin/python -m gabru_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
        ...
        monkeypatch.setattr(gateway_cli, "_profile_arg", lambda gabru_home=None: "--profile orcha")
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``gabru update --gateway``, the exit code marker must be
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point GABRU_HOME at a temp dir so the marker file lands there
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))
        import gabru_cli.config as _cfg
        monkeypatch.setattr(_cfg, "get_gabru_home", lambda: gabru_home)
        # Also patch the module-level ref used by cmd_update
        import gabru_cli.main as _main_mod
        monkeypatch.setattr(_main_mod, "get_gabru_home", lambda: gabru_home)
        ...
        exit_code_path = gabru_home / ".update_exit_code"
...
class TestCmdUpdateLegacyGatewayWarning:
    """Tests for the legacy gabru.service warning printed by `gabru update`.

    Users who installed Gabru before the service rename often have a
    dormant ``gabru.service`` that starts flap-fighting the current
    ``gabru-gateway.service`` after PR #5646. Every ``gabru update``
    should remind them to run ``gabru gateway migrate-legacy`` until
    they do.
    """
    ...
    def test_update_prints_legacy_warning_when_detected(
        ...
        user_dir.mkdir()
        system_dir.mkdir()
        legacy_path = user_dir / "gabru.service"
        ...
        captured = capsys.readouterr().out
        assert "Legacy Gabru gateway unit(s) detected" in captured
        assert "gabru.service" in captured
        assert "gabru gateway migrate-legacy" in captured
    ...
    def test_update_silent_when_no_legacy_units(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Gabru gateway" not in captured
    ...
    def test_update_does_not_flag_profile_units(
        ...
        """Profile units (gabru-gateway-coder.service) must not trigger the warning.

        This is the core safety invariant: the legacy allowlist is
        ``gabru.service`` only, no globs.
        """
        user_dir = tmp_path / "user"
        system_dir = tmp_path / "system"
        user_dir.mkdir()
        ...
        (user_dir / "gabru-gateway-coder.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        (user_dir / "gabru-gateway.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        ...
        captured = capsys.readouterr().out
        assert "Legacy Gabru gateway" not in captured
        assert "gabru-gateway-coder.service" not in captured  # not flagged
    ...
    def test_update_lists_system_scope_unit_with_sudo_hint(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Gabru gateway" in captured
