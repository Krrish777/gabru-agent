"""Tests for gabru_cli.web_server and related config utilities."""
...
class TestReloadEnv:
    ...
    def test_adds_new_vars(self, tmp_path):
        ...
        with patch("gabru_cli.config.get_env_path", return_value=env_file):
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Gabru vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Gabru env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch("gabru_cli.config.get_env_path", return_value=env_file):
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_gabru_home):
        """Create a TestClient and isolate the state DB under the test GABRU_HOME."""
        try:
        ...
        from gabru_cli.web_server import app, _SESSION_TOKEN

        monkeypatch.setattr(gabru_state, "DEFAULT_DB_PATH", get_gabru_home() / "state.db")
    ...
    def test_get_status(self):
        ...
        assert "gabru_home" in data
        assert "active_sessions" in data

    def test_get_status_filters_unconfigured_gateway_platforms(self, monkeypatch):
        import gateway.config as gateway_config
        import gabru_cli.web_server as web_server
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from gabru_cli.config import save_env_value
        from gabru_cli.web_server import _SESSION_TOKEN
    ...
    def test_reveal_env_var_no_token(self, tmp_path):
        ...
        from starlette.testclient import TestClient
        from gabru_cli.web_server import app
        from gabru_cli.config import save_env_value
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/gabru_cli/web_server.py")
        assert resp.status_code in (200, 404)
        if resp.status_code == 200:
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from gabru_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from gabru_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_gabru_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import gabru_cli.skills_config as skills_config
        import gabru_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import gabru_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from gabru_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from gabru_cli.web_server import _denormalize_config_from_web
        from gabru_cli.config import save_config
...
class TestModelInfoEndpoint:
    ...
    def test_model_info_with_dict_config(self, monkeypatch):
        import gabru_cli.web_server as ws
