...
def test_openrouter_base_url_applies_or_headers(mock_openai):
    ...
    assert headers["HTTP-Referer"] == "https://gabru-agent."
    assert headers["X-OpenRouter-Title"] == "Gabru Agent"
...
def test_ai_gateway_base_url_applies_attribution_headers(mock_openai):
    ...
    headers = agent._client_kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://gabru-agent."
    assert headers["X-Title"] == "Gabru Agent"
    assert headers["User-Agent"].startswith("GabruAgent/")
