...
SCRIPT_PATH = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_gabru.py"
)


def load_module():
    spec = importlib.util.spec_from_file_location("openclaw_to_gabru", SCRIPT_PATH)
...
"""
    ...
    assert "Tyler Williams > Active Projects: Gabru Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `gabru cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # gabru_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.gabru/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "gabru_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Gabru prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Gabru to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Gabru config is great"
    assert mod.rebrand_text("openclaw should always respond concisely") == "Gabru should always respond concisely"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Gabru uses tools well"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Gabru remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "Gabru prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Gabru was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "Gabru uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Gabru said to ask Gabru about Gabru settings"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Gabru" in result
