"""Tests for gabru_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    gabru_logging._logging_initialized = False
    ...
    gabru_logging._install_session_record_factory()
    yield
    ...
    for h in list(root.handlers):
        if h not in pre_existing:
            ...
            h.close()
    gabru_logging._logging_initialized = False
    gabru_logging.clear_session_context()


@pytest.fixture
def gabru_home(tmp_path, monkeypatch):
    """Provide an isolated GABRU_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_gabru_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["GABRU_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, gabru_home):
        log_dir = gabru_logging.setup_logging(gabru_home=gabru_home)
        assert log_dir == gabru_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, gabru_home):
    ...
    def test_creates_errors_log_handler(self, gabru_home):
        gabru_logging.setup_logging(gabru_home=gabru_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, gabru_home):
        gabru_logging.setup_logging(gabru_home=gabru_home)
        gabru_logging.setup_logging(gabru_home=gabru_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, gabru_home):
        gabru_logging.setup_logging(gabru_home=gabru_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        gabru_logging.setup_logging(gabru_home=gabru_home, force=True)
    ...
    def test_custom_log_level(self, gabru_home):
        gabru_logging.setup_logging(gabru_home=gabru_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, gabru_home):
        gabru_logging.setup_logging(
            gabru_home=gabru_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, gabru_home):
        gabru_logging.setup_logging(gabru_home=gabru_home)
    ...
    def test_writes_to_agent_log(self, gabru_home):
        gabru_logging.setup_logging(gabru_home=gabru_home)

        test_logger = logging.getLogger("test_gabru_logging.write_test")
        ...
        agent_log = gabru_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, gabru_home):
        gabru_logging.setup_logging(gabru_home=gabru_home)

        test_logger = logging.getLogger("test_gabru_logging.warning_test")
        ...
        agent_log = gabru_home / "logs" / "agent.log"
        errors_log = gabru_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, gabru_home):
        gabru_logging.setup_logging(gabru_home=gabru_home)

        test_logger = logging.getLogger("test_gabru_logging.info_test")
    ...
    def test_explicit_params_override_config(self, gabru_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (gabru_home / "config.yaml").write_text(yaml.dump(config))

        gabru_logging.setup_logging(gabru_home=gabru_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, gabru_home):
        """The custom record factory injects session_tag on all records."""
        gabru_logging.setup_logging(gabru_home=gabru_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_gabru_session_injector", False), (
            "Record factory should have _gabru_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, gabru_home):
        gabru_logging.setup_logging(gabru_home=gabru_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, gabru_home):
        gabru_logging.setup_logging(gabru_home=gabru_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, gabru_home):
        ...
        gw_log = gabru_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, gabru_home):
    ...
    def test_agent_log_still_receives_all(self, gabru_home):
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, gabru_home):
        """When session context is set, log lines include [session_id]."""
        gabru_logging.setup_logging(gabru_home=gabru_home)
        gabru_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, gabru_home):
    ...
    def test_clear_session_context(self, gabru_home):
        """After clearing, session tag disappears."""
        gabru_logging.setup_logging(gabru_home=gabru_home)
        gabru_logging.set_session_context("xyz789")
        gabru_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, gabru_home):
        ...
        def thread_b():
            gabru_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        gabru_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        gabru_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = gabru_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = gabru_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in gabru_logging.COMPONENT_PREFIXES
        assert ("gateway",) == gabru_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = gabru_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == gabru_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = gabru_logging.COMPONENT_PREFIXES["cli"]
        assert "gabru_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == gabru_logging.COMPONENT_PREFIXES["cron"]


class TestSetupVerboseLogging:
    """setup_verbose_logging() adds a DEBUG-level console handler."""

    def test_adds_stream_handler(self, gabru_home):
        gabru_logging.setup_logging(gabru_home=gabru_home)
        gabru_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_gabru_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, gabru_home):
        gabru_logging.setup_logging(gabru_home=gabru_home)
        gabru_logging.setup_verbose_logging()
        gabru_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        gabru_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = gabru_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        gabru_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("gabru_cli.config.is_managed", return_value=True):
                gabru_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, gabru_home):
        level, max_size, backup = gabru_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, gabru_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, gabru_home):
        import yaml
        config = {"model": "test"}
        (gabru_home / "config.yaml").write_text(yaml.dump(config))
