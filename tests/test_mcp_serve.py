"""
Tests for mcp_serve — Gabru MCP server.
...
def _isolate_gabru_home(tmp_path, monkeypatch):
    """Redirect GABRU_HOME to a temp directory."""
    monkeypatch.setenv("GABRU_HOME", str(tmp_path))
    try:
        import gabru_constants
        monkeypatch.setattr(gabru_constants, "get_gabru_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking gabru_state schema."""
