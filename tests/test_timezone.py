"""
Tests for timezone support (gabru_time module + integration points).
...
def _reset_gabru_time_cache():
    """Reset the gabru_time module cache (replacement for removed reset_cache)."""
    gabru_time._cached_tz = None
    gabru_time._cached_tz_name = None
    gabru_time._cache_resolved = False


# =========================================================================
# gabru_time.now() — core helper
# =========================================================================

class TestGabruTimeNow:
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["GABRU_TIMEZONE"] = "Asia/Kolkata"
        result = gabru_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["GABRU_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="gabru_time"):
            result = gabru_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["GABRU_TIMEZONE"] = "Europe/London"
        tz = gabru_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["GABRU_TIMEZONE"] = "Not/A/Timezone"
        tz = gabru_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When GABRU_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) GABRU_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("GABRU_TIMEZONE=" + os.environ.get("GABRU_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "GABRU_TIMEZONE=NOT_SET" in result["output"], (
            "GABRU_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When GABRU_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("GABRU_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=gabru_tz) which shifted
        absolute time when system-local tz != Gabru tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_gabru_tz(self):
        """Already-aware datetimes should be normalized to Gabru tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Gabru tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Gabru tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Gabru is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Gabru tz
        ...
        # Use a Gabru timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _gabru_now's wall time — this would
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Gabru timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["GABRU_TIMEZONE"] = "US/Eastern"
        _reset_gabru_time_cache()
