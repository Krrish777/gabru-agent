...
class TestToolsetConsistency:
    ...
    def test_gabru_platforms_share_core_tools(self):
        """All gabru-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord_server`` on
        gabru-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["gabru-cli", "gabru-telegram", "gabru-discord", "gabru-whatsapp", "gabru-slack", "gabru-signal", "gabru-homeassistant"]
