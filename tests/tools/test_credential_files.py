...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        gabru_home.mkdir()
        (gabru_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"GABRU_HOME": str(gabru_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(gabru_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.gabru/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        (gabru_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        (gabru_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        (gabru_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.gabru/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        gabru_home = tmp_path / ".gabru"
        (gabru_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"GABRU_HOME": str(gabru_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.gabru")

        assert mounts[0]["container_path"] == "/home/user/.gabru/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.gabru/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.gabru/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape GABRU_HOME."""
        monkeypatch.setenv("GABRU_HOME", str(tmp_path / ".gabru"))
        (tmp_path / ".gabru").mkdir()

        # Create a sensitive file one level above gabru_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside gabru_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside GABRU_HOME must still be registered."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))
        (gabru_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_gabru_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of GABRU_HOME must be allowed."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        subdir = gabru_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside GABRU_HOME pointing outside must be rejected."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))

        # Create a sensitive file outside gabru_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside gabru_home pointing outside
        symlink = gabru_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes GABRU_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, gabru_home: Path, cred_files: list):
        import yaml
        config_path = gabru_home / "config.yaml"
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape GABRU_HOME."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))
        ...
        sensitive.write_text("{}")
        self._write_config(gabru_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(gabru_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside GABRU_HOME via config must still mount."""
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))

        (gabru_home / "oauth.json").write_text("{}")
        self._write_config(gabru_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        (gabru_home / "cache" / "documents").mkdir(parents=True)
        (gabru_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.gabru/cache/documents" in paths
        assert "/root/.gabru/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.gabru/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        gabru_home = tmp_path / ".gabru"
        gabru_home.mkdir()
        # Use legacy dir name — get_gabru_dir prefers old if it exists
        (gabru_home / "document_cache").mkdir()
        (gabru_home / "image_cache").mkdir()
        monkeypatch.setenv("GABRU_HOME", str(gabru_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(gabru_home / "document_cache") in host_paths
        assert str(gabru_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.gabru/cache/documents" in container_paths
        assert "/root/.gabru/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        gabru_home = tmp_path / ".gabru"
        doc_dir = gabru_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        gabru_home = tmp_path / ".gabru"
        ss_dir = gabru_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.gabru/cache/screenshots/session_abc/screen1.png"
