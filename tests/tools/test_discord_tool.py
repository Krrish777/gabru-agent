...
# ---------------------------------------------------------------------------
# Toolset: discord_server only in gabru-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_server_in_gabru_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord_server" in TOOLSETS["gabru-discord"]["tools"]

    def test_discord_server_not_in_core_tools(self):
        from toolsets import _GABRU_CORE_TOOLS
        assert "discord_server" not in _GABRU_CORE_TOOLS

    def test_discord_server_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name == "gabru-discord":
                continue
            # The gateway toolset might include it if it unions all platform tools
            if name == "gabru-gateway":
                continue
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("gabru_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_server_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["gabru-discord"], quiet_mode=True)
        discord_tool = next(
