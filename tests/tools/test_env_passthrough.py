...
class TestTerminalIntegration:
    ...
    def test_blocklisted_var_blocked_by_default(self):
        from tools.environments.local import _GABRU_PROVIDER_ENV_BLOCKLIST, _sanitize_subprocess_env

        # Pick a var we know is in the blocklist
        blocked_var = next(iter(_GABRU_PROVIDER_ENV_BLOCKLIST))
    ...
    def test_passthrough_cannot_override_provider_blocklist(self):
        ...
        Gabru provider credentials — that was the bypass where a skill
    ...
    def test_non_gabru_api_key_still_registerable(self):
        """Third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
        Gabru provider credentials and must still pass through — skills
