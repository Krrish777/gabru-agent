...
Validates that:
1. The installed atroposlib API still matches Gabru's expectations
2. Gabru's parser registry remains compatible with ManagedServer parsing
3. GabruAgentBaseEnv wires the selected parser into ServerManager correctly

These tests verify the contract between gabru-agent's environments/ code
...
class TestManagedServerAPI:
    """Test that ManagedServer's API matches what gabru-agent expects."""
...
class TestParserCompatibility:
    """Test that gabru-agent's parsers match ManagedServer's expectations."""
...
class TestBaseEnvCompatibility:
    """Test that gabru_base_env.py's tool-parser wiring matches the current API."""

    def test_gabru_base_env_sets_server_manager_tool_parser(self):
        """Gabru wires parser selection through ServerManager.tool_parser."""
        import ast

        base_env_path = Path(__file__).parent.parent.parent / "environments" / "gabru_base_env.py"
        ...
        assert found_assignment, (
            "gabru_base_env.py should set self.server.tool_parser from config.tool_call_parser"
        )

    def test_gabru_base_env_uses_config_tool_call_parser(self):
        """Verify gabru_base_env uses the config field rather than a local parser instance."""
        base_env_path = Path(__file__).parent.parent.parent / "environments" / "gabru_base_env.py"
        source = base_env_path.read_text()
