...
def _restore_tool_modules():
    original_gabru_home = os.environ.get("GABRU_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_gabru_home is None:
            os.environ.pop("GABRU_HOME", None)
        else:
            os.environ["GABRU_HOME"] = original_gabru_home
        _reset_modules(("tools", "gabru_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    gabru_cli = types.ModuleType("gabru_cli")
    gabru_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["gabru_cli"] = gabru_cli
    gabru_home = tmp_path / "gabru-home"
    os.environ["GABRU_HOME"] = str(gabru_home)
    sys.modules["gabru_cli.config"] = types.SimpleNamespace(
        get_gabru_home=lambda: gabru_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="gabru-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": gabru_home / "modal_snapshots.json",
