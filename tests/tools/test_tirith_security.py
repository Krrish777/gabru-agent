...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._gabru_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_gabru_bin(self):
        """After _INSTALL_FAILED, manual install in GABRU_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        gabru_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(gabru_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(gabru_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._gabru_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == gabru_bin
            assert _tirith_mod._resolved_path == gabru_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# GABRU_HOME isolation
# ---------------------------------------------------------------------------

class TestGabruHomeIsolation:
    def test_gabru_bin_dir_respects_gabru_home(self):
        """_gabru_bin_dir must use GABRU_HOME, not hardcoded ~/.gabru."""
        from tools.tirith_security import _gabru_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"GABRU_HOME": tmpdir}):
            result = _gabru_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_gabru_home(self):
        """_failure_marker_path must use GABRU_HOME, not hardcoded ~/.gabru."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"GABRU_HOME": "/custom/gabru"}):
            result = _failure_marker_path()
        assert result == "/custom/gabru/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets GABRU_HOME; verify it's active."""
        gabru_home = os.getenv("GABRU_HOME")
        assert gabru_home is not None, "GABRU_HOME should be set by conftest"
        assert "gabru_test" in gabru_home, "Should point to test temp dir"

    def test_get_gabru_home_fallback(self):
        """Without GABRU_HOME set, falls back to ~/.gabru."""
        from tools.tirith_security import _get_gabru_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove GABRU_HOME entirely
            os.environ.pop("GABRU_HOME", None)
            result = _get_gabru_home()
        assert result == os.path.join(os.path.expanduser("~"), ".gabru")
