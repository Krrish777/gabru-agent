...
DANGEROUS_PATTERNS = [
    ...
    (r'\bgabru\s+gateway\s+(stop|restart)\b', "stop/restart gabru gateway (kills running agents)"),
    (r'\bgabru\s+update\b', "gabru update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart gabru-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart gabru-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(gabru|gateway|cli\.py)\b', "kill gabru/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill gabru` but not
    # `kill -9 $(pgrep -f gabru)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("GABRU_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("GABRU_INTERACTIVE")
    is_gateway = os.getenv("GABRU_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        # Cron sessions: respect cron_mode config
        if os.getenv("GABRU_CRON_SESSION"):
    ...
    if is_gateway or os.getenv("GABRU_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("GABRU_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("GABRU_INTERACTIVE")
    is_gateway = os.getenv("GABRU_GATEWAY_SESSION")
    is_ask = os.getenv("GABRU_EXEC_ASK")
