...
Architecture:
    ~/.gabru/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        GABRU_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = get_gabru_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("GABRU_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(shadow_repo: Path, working_dir: str) -> dict:
    ...
    The shadow repo is internal Gabru infrastructure — it must NOT inherit
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "gabru@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "Gabru Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "GABRU_WORKDIR").write_text(
