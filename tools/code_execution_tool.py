...
Lets the LLM write a Python script that calls Gabru tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `gabru_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `gabru_tools.py` with file-based RPC stubs
...
def generate_gabru_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the gabru_tools.py stub module.
...
"""Auto-generated Gabru tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["GABRU_RPC_SOCKET"])
...
"""Auto-generated Gabru tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, time

_RPC_DIR = os.environ.get("GABRU_RPC_DIR") or os.path.join(tempfile.gettempdir(), "gabru_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    The script and the generated gabru_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/gabru_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_gabru_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/gabru_tools.py", tools_src)
        ...
        env_prefix = (
            f"GABRU_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("GABRU_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with gabru_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="gabru_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"gabru_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated gabru_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_gabru_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "gabru_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["GABRU_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the gabru-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from gabru_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _gabru_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _gabru_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # GABRU_TIMEZONE is an internal Gabru setting and must not leak
        # into child processes.
        _tz_name = os.getenv("GABRU_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("GABRU_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {GABRU_HOME}/home/ when that directory exists.
        from gabru_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.gabru/.env')).
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (gabru-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``gabru tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and gabru-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.gabru/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Gabru tools programmatically. "
        ...
        f"Available via `from gabru_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into gabru_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from gabru_tools import {import_str}` "
