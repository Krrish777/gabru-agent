...
def _resolve_gabru_home() -> Path:
    from gabru_constants import get_gabru_home
    return get_gabru_home()


def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.gabru",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``GABRU_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside GABRU_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    gabru_home = _resolve_gabru_home()

    # Reject absolute paths — they bypass the GABRU_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to GABRU_HOME)",
            relative_path,
        )
        return False

    host_path = gabru_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape GABRU_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, gabru_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.gabru/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="gabru-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching gabru_constants.get_gabru_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_gabru_dir()`` for backward compatibility with old directory layouts.
    """
    from gabru_constants import get_gabru_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_gabru_dir(new_subpath, old_name)
        if host_dir.is_dir():
