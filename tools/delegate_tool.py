...
# (gabru-* prefixed), and scenario toolsets.
...
def _build_child_agent(
    ...
    try:
        ...
        if delegation_effort:
            from gabru_constants import parse_reasoning_effort
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'gabru auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (gabru_cli/config.py load_config()) so that
