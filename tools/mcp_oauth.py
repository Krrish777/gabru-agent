...
This module provides the glue:
    - ``GabruTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Gabru Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses GABRU_HOME so each profile gets its own OAuth tokens.
    Layout: ``GABRU_HOME/mcp-tokens/``
    """
    try:
        from gabru_constants import get_gabru_home
        base = Path(get_gabru_home())
    except ImportError:
        base = Path(os.environ.get("GABRU_HOME", str(Path.home() / ".gabru")))
...
# ---------------------------------------------------------------------------
# GabruTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class GabruTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        GABRU_HOME/mcp-tokens/<server_name>.json         -- tokens
        GABRU_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Gabru records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Gabru.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Gabru Agent")
    scope = cfg.get("scope")
