...
`mcp_tool.py`, and `gabru_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# GabruMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_gabru_provider_class() -> Optional[type]:
    ...
    class GabruMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._gabru_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`GabruTokenStorage` persisting an absolute
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._gabru_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._gabru_server_name, asm.token_endpoint,
                        )
                        break
    ...
    return GabruMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_GABRU_PROVIDER_CLS: Optional[type] = _make_gabru_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`GabruMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _GABRU_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = GabruTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``gabru mcp remove <name>`` and (indirectly) by
        ``gabru mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
