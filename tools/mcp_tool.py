...
discovers their tools, and registers them into the gabru-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.gabru/config.yaml under the ``mcp_servers`` key.
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            gabru_home = os.path.expanduser(
                os.getenv(
                    "GABRU_HOME", os.path.join(os.path.expanduser("~"), ".gabru")
                )
            )
            candidates = [
                os.path.join(gabru_home, "node", "bin", resolved_command),
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `gabru mcp login {server_name}` (or delete the tokens "
            f"file under ~/.gabru/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Gabru config file.
    ...
    ``os.environ`` (which includes ``~/.gabru/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Gabru's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Gabru registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``gabru tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Gabru registry.
