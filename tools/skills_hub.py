#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Gabru Skills Hub.
...
Used by gabru_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            gabru_meta = metadata.get("gabru", {})
            if isinstance(gabru_meta, dict):
                tags = gabru_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by ) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.gabru/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    def __init__(self):
        from gabru_constants import get_optional_skills_dir
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                gabru_meta = meta_block.get("gabru", {})
                if isinstance(gabru_meta, dict):
                    tags = gabru_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Gabru centralized index source
# ---------------------------------------------------------------------------

GABRU_INDEX_URL = "https://gabru-agent./docs/api/skills-index.json"
GABRU_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "gabru-index.json"
GABRU_INDEX_TTL = 6 * 3600  # 6 hours


def _load_gabru_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for GABRU_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if GABRU_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - GABRU_INDEX_CACHE_FILE.stat().st_mtime
            if age < GABRU_INDEX_TTL:
                return json.loads(GABRU_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(GABRU_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Gabru index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Gabru index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        GABRU_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        GABRU_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class GabruIndexSource(SkillSource):
    """Skill source backed by the centralized Gabru Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "gabru-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "gabru-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        GabruIndexSource(auth=auth), # Centralized index (search + resolved install paths)
