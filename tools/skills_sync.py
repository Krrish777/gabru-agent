...
Copies bundled skills from the repo's skills/ directory into ~/.gabru/skills/
...
The manifest lives at ~/.gabru/skills/.bundled_manifest.
"""
...
def _get_bundled_dir() -> Path:
    ...
    Checks GABRU_BUNDLED_SKILLS env var first (set by Nix wrapper),
    then falls back to the relative path from this source file.
    """
    env_override = os.getenv("GABRU_BUNDLED_SKILLS")
    if env_override:
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.gabru/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.gabru/skills/ using the manifest.
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `gabru skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `gabru update` runs "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.gabru/skills/ ...")
