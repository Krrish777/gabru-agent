...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Gabru exit
...
# display_gabru_home imported lazily at call site (stale-module safety during gabru update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of gabru directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "gabru-*")):
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("GABRU_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (GABRU_INTERACTIVE=1).
    ...
    try:
        os.environ["GABRU_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "GABRU_SPINNER_PAUSE" in os.environ:
            del os.environ["GABRU_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (GABRU_INTERACTIVE=1):
    ...
    if not has_configured_password and not sudo_password and os.getenv("GABRU_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Gabru can track lifecycle and output.
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.gabru/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Modal credentials/config were found. Log in with `gabru model` or "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(command):
        return (
            ...
            "Use terminal(background=true) so Gabru can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if _gw_platform:
                        ...
                        _gw_thread_id = _gse("GABRU_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("GABRU_SESSION_USER_ID", "")
                        _gw_user_name = _gse("GABRU_SESSION_USER_NAME", "")
            ...
            try:
                from gabru_cli.plugins import invoke_hook
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["managed_mode_blocked"]:
                    logger.error(
                        ...
                        "a paid Nous subscription is required for the Tool Gateway and no direct "
                        "Modal credentials/config were found. Log in with `gabru model` "
