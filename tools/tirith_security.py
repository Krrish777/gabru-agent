...
it is automatically downloaded from GitHub releases to $GABRU_HOME/bin/tirith.
...
def _get_gabru_home() -> str:
    """Return the Gabru home directory, respecting GABRU_HOME env var."""
    return str(get_gabru_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_gabru_home(), ".tirith-install-failed")
...
def _gabru_bin_dir() -> str:
    """Return $GABRU_HOME/bin, creating it if needed."""
    d = os.path.join(_get_gabru_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $GABRU_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_gabru_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $GABRU_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $GABRU_HOME/bin/tirith
    ...
    gabru_bin = os.path.join(_gabru_bin_dir(), "tirith")
    if os.path.isfile(gabru_bin) and os.access(gabru_bin, os.X_OK):
        _resolved_path = gabru_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return gabru_bin
